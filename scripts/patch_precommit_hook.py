...
def resolve_hooks_dir() -> pathlib.Path:
    """Resolve the git hooks directory, working in a plain checkout AND a worktree.

    In a worktree `.git` is a *file* pointing at the real gitdir, so the literal
    `.git/hooks` path does not exist and the old hard-coded lookup failed the whole
    `make install` target (#386). `git rev-parse --git-path hooks` returns the shared
    main-repo hooks directory correctly in both layouts; fall back to `.git/hooks`
    only if git is unavailable.
    """
    try:
        out = subprocess.run(
...
def main() -> int:
    ...
    if not hooks_dir.exists():
        print(
            f"❌  git hooks directory not found ({hooks_dir}) — run inside the repo (or a worktree of it).",
            file=sys.stderr,
        )
        return 1
    ...
    if not hooks:
        print(f"⚠️   No hooks found in {hooks_dir}.", file=sys.stderr)
        return 0
