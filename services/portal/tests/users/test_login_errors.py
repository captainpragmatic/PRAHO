...
class LoginErrorsTestCase(SimpleTestCase):
    ...
    def test_rate_limited_login_shows_throttle_message_not_invalid_credentials(self):
        ...
        self.assertContains(response, "Too many login attempts. Please try again in 30 seconds.")
...
class ChangePasswordRateLimitTestCase(SimpleTestCase):
    ...
    def test_password_change_rate_limited_shows_throttle_message(self):
        """When update_customer_password raises rate-limited PlatformAPIError, show throttle msg."""

        self._login_session()
        with patch("apps.users.views.api_client") as mock_api:
            mock_api.update_customer_password.side_effect = PlatformAPIError(
                "Too many requests",
                status_code=429,
                retry_after=60,
                is_rate_limited=True,
