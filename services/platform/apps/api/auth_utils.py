...
def _validate_hmac_authentication(request: HttpRequest) -> tuple[bool, Response | None]:
    ...
    if not hasattr(request, "_portal_authenticated"):
        ...
        return False, Response({"success": False, "error": "Authentication required"}, status=401)

    if not hasattr(request, "user") or not request.user.is_authenticated:
        logger.warning("🔥 [API Security] No authenticated user in HMAC request")
        return False, Response({"success": False, "error": "User authentication required"}, status=401)
...
def _get_specific_customer(user: "User", required_customer_id: int) -> tuple[Customer | None, Response | None]:
    ...
    try:
        ...
        if not membership:
            logger.warning(
                f"🔥 [API Security] User {user.email} attempted access to customer {required_customer_id} without membership"
            )
            return None, Response({"success": False, "error": "Access denied to customer data"}, status=403)
        ...
        return None, Response({"success": False, "error": "Customer not found"}, status=404)


def _get_primary_customer(user: "User") -> tuple[Customer | None, Response | None]:
    """Get user's primary customer"""
    membership = (
        CustomerMembership.objects.filter(user=user, is_primary=True, is_active=True).select_related("customer").first()
    )

    if not membership:
        logger.warning(f"🔥 [API Security] User {user.email} has no primary customer membership")
        return None, Response({"success": False, "error": "No customer access found"}, status=403)
...
def get_authenticated_customer_from_request(
    request: HttpRequest, required_customer_id: int | None = None
...
def get_customer_id_from_request(request: HttpRequest) -> tuple[int | None, Response | None]:
    ...
    return None, Response(
        {"success": False, "error": "Use HMAC-signed request body with customer_id (query param deprecated)"},
        status=400,
    )


def require_customer_access(
    request: HttpRequest, customer_id: int | None = None
) -> tuple[Customer | None, Response | None]:
    """
...
def get_customer_from_hmac_context(request: HttpRequest) -> tuple[Customer | None, Response | None]:
    ...
    try:
        ...
        if hasattr(request, "user") and request.user.is_authenticated:
            membership = CustomerMembership.objects.filter(user=request.user, customer=customer, is_active=True).first()

            if not membership:
                logger.warning(
                    f"🔥 [API Security] User {request.user.email} attempted access to customer {customer_id} without membership"
                )
