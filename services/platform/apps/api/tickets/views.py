...
def customer_ticket_detail_api(request: HttpRequest, customer: Customer, ticket_id: int) -> Response:
    ...
    try:
        ...
        response_data = {"success": True, "data": {"ticket": serializer.data, "permissions": permissions}}
...
def customer_ticket_create_api(request: HttpRequest, customer: Customer) -> Response:
    ...
    try:
        ...
        ticket_data = {k: v for k, v in request_data.items() if k not in ["customer_id", "action", "timestamp"]}
...
def customer_tickets_summary_api(request: HttpRequest, customer: Customer) -> Response:
    ...
    try:
        ...
        response_data = {"success": True, "data": summary_data}
...
def support_categories_api(request: HttpRequest) -> Response:
    ...
    try:
        ...
        response_data = {"success": True, "data": {"categories": serializer.data}}
...
def ticket_attachment_download_api(
    request: HttpRequest, customer: Customer, ticket_id: int, attachment_id: int
) -> HttpResponse:
    """
    📎 Ticket Attachment Download API
    ...
    try:
        ...
        logger.info(
            f"✅ [API] Ticket attachment download: {attachment.filename}, ticket={attachment.ticket.ticket_number}"
        )
        return response
