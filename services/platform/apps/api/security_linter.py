        ...
        class APIVisitor(ast.NodeVisitor):
            ...
            def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
                ...
                for decorator in node.decorator_list:
                    if isinstance(decorator, ast.Call) and (
                        hasattr(decorator.func, "id") and decorator.func.id == "api_view"
                    ):
            ...
            def _check_function_violations(self, node: ast.FunctionDef) -> None:
                ...
                has_customer_data = any(
                    re.search(pattern, function_source) for pattern in self.linter.customer_data_patterns
                )

                has_proper_auth = any(re.search(pattern, function_source) for pattern in self.linter.auth_patterns)
