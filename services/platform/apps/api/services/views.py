...
def customer_services_api(request: HttpRequest, customer: Customer) -> Response:
    ...
    try:
        ...
        return Response({"success": False, "error": "Unable to fetch services list"}, status=500)
...
def customer_service_detail_api(request: HttpRequest, customer: Customer, service_id: int) -> Response:
    ...
    try:
        ...
        try:
            ...
            return Response({"success": False, "error": "Service not found or access denied"}, status=404)
        ...
        return Response({"success": True, "data": {"service": serializer.data, "usage_trends": usage_trends}})
...
def customer_services_summary_api(request: HttpRequest, customer: Customer) -> Response:
    ...
    try:
        ...
        return Response({"success": True, "data": {"summary": summary_data}})
...
def available_service_plans_api(request: HttpRequest) -> Response:
    ...
    Public service plan catalog for pricing pages.
    ...
    try:
        ...
        return Response({"success": False, "error": "Unable to fetch service plans"}, status=500)
...
def update_service_auto_renew_api(request: HttpRequest, customer: Customer, service_id: int) -> Response:
    ...
    try:
        ...
        return Response({"success": False, "error": "Unable to update auto-renew setting"}, status=500)
    ...
    try:
        ...
        usage_data = {
            "current_usage": {
                ...
                "disk_usage_percentage": round(
                    (service.disk_usage_mb / 1024) / (service.service_plan.disk_space_gb or 1) * 100, 1
                )
                ...
                "bandwidth_usage_percentage": round(
                    (service.bandwidth_usage_mb / 1024) / (service.service_plan.bandwidth_gb or 1) * 100, 1
                )
        ...
        # Add usage alerts
        if service.service_plan.disk_space_gb and (service.disk_usage_mb / 1024) > (
            service.service_plan.disk_space_gb * 0.8
        ):
            usage_data["alerts"].append(
                {"type": "warning", "message": "Disk usage is above 80% of limit", "resource": "disk"}
            )

        if service.service_plan.bandwidth_gb and (service.bandwidth_usage_mb / 1024) > (
            service.service_plan.bandwidth_gb * 0.8
        ):
            usage_data["alerts"].append(
                {"type": "warning", "message": "Bandwidth usage is above 80% of limit", "resource": "bandwidth"}
            )

        return Response({"success": True, "data": {"usage": usage_data}})

    except Exception as e:
        logger.error(f"🔥 [Services API] Error fetching usage stats for service {service_id}: {e}")
        return Response({"success": False, "error": "Unable to fetch usage statistics"}, status=500)
