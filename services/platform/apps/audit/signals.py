...
def _get_action_category_severity(action: str) -> tuple[str, str, bool, bool]:
    ...
    action_config = {
        ...
        ("password_",): (
            "authentication",
            lambda a: "high" if a in ["password_compromised", "password_strength_weak"] else "medium",
            True,
            lambda a: a == "password_compromised",
        ),
        ("2fa_",): (
            "authentication",
            lambda a: "high" if a in ["2fa_disabled", "2fa_admin_reset"] else "medium",
            True,
            lambda a: a in ["2fa_disabled", "2fa_admin_reset"],
        ),
        ("privacy_", "gdpr_", "marketing_consent"): ("privacy", "high", True, lambda a: a.endswith("_withdrawn")),
        ("customer_",): ("authorization", "medium", False, lambda a: a.endswith("_revoked")),
        ("security_", "suspicious_", "brute_force", "malicious_"): ("security_event", "critical", True, lambda a: True),
        ("data_export", "data_deletion", "data_breach"): ("data_protection", "high", True, lambda a: True),
        ("system_", "backup_", "configuration_", "user_impersonation"): ("system_admin", "high", True, lambda a: True),
    }
                        ...
                        _create_audit_event(
                            AuditEventCreationData(
                                ...
                                metadata={
                                    ...
                                    "requires_review": not instance.two_factor_enabled,
                                },
                            )
                        )
...
def audit_customer_membership_changes(
    sender: type[CustomerMembership], instance: CustomerMembership, created: bool, **kwargs: Any
) -> None:
...
def audit_customer_membership_deletion(
    sender: type[CustomerMembership], instance: CustomerMembership, **kwargs: Any
) -> None:
...
def audit_privacy_settings_change(
    sender: Any,
    user: User,
...
def audit_api_key_generation(
    sender: Any, user: User, api_key_info: dict[str, Any], request: HttpRequest | None = None, **kwargs: Any
) -> None:
...
def audit_cookie_consent_change(
    ...
    **kwargs: Any,
    ...
    """
    from .services import (  # Circular: signals<->services  # noqa: PLC0415  # Deferred: avoids circular import
        AuditService,
