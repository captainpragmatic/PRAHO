...
def generate_e_factura_xml(invoice: Invoice) -> str:
    ...
    try:
        ...
        due_date = xml_escape(invoice.due_date.strftime('%Y-%m-%d') if hasattr(invoice, 'due_date') and invoice.due_date else '')
        ...
        customer_vat = xml_escape(customer.vat_number if customer and hasattr(customer, 'vat_number') and customer.vat_number else '')
