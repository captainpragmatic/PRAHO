...
def change_user_role(request: HttpRequest, customer_id: int, membership_id: int) -> HttpResponse:
    ...
    if membership.role == "owner" and new_role != "owner":
        owner_count = CustomerMembership.objects.filter(
            customer=customer, 
            role="owner"
        ).count()
        ...
        if owner_count <= 1:
            messages.error(request, _(
                "Cannot change role: {} is the only owner. "
    ...
    ).format(membership.user.email, old_role, membership.get_role_display()))
...
def remove_user(request: HttpRequest, customer_id: int, membership_id: int) -> HttpResponse:
    ...
    if membership.role == "owner":
        ...
        if owner_count <= 1:
            messages.error(request, _(
                "Cannot remove user: {} is the only owner. "
                "Promote another user to owner first."
            ).format(membership.user.email))
            return redirect("customers:detail", customer_id=customer.id)
