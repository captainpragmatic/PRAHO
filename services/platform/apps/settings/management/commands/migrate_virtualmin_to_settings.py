...
@dataclass
class CredentialMigrationData:
    """Data structure for credential migration parameters"""

    vault: Any
    hostname: str
    credential_info: tuple[str, str, str]
    username: str
    password: str
...
class Command(BaseCommand):
    ...
    def _analyze_current_state(self) -> None:
        ...
        env_vars = [
            ...
            "VIRTUALMIN_MASTER_USERNAME",
            "VIRTUALMIN_MASTER_PASSWORD",
            "VIRTUALMIN_SSH_USERNAME",
            "VIRTUALMIN_SSH_PRIVATE_KEY_PATH",
            "VIRTUALMIN_SSH_PASSWORD",
            "VIRTUALMIN_AUTH_HEALTH_CHECK_INTERVAL",
        ...
        try:
            ...
            provisioning_settings = SystemSetting.objects.filter(
                key__startswith="virtualmin.", category=provisioning_category
            )
    ...
    def _migrate_operational_settings(self, force: bool = False) -> None:
        ...
        settings_to_migrate = [
            ...
            (
                ...
                os.environ.get("VIRTUALMIN_SSL_VERIFY", "True").lower() == "true",
                "boolean",
                "SSL verification",
            ),
            (
                "virtualmin.auth_health_check_interval_seconds",
                int(os.environ.get("VIRTUALMIN_AUTH_HEALTH_CHECK_INTERVAL", "3600")),
                "integer",
                "Health check interval",
            ),
            (
                "virtualmin.auth_fallback_enabled",
                os.environ.get("VIRTUALMIN_AUTH_FALLBACK_ENABLED", "true").lower() == "true",
                "boolean",
                "Fallback authentication",
            ),
            (
                "virtualmin.ssh_username",
                os.environ.get("VIRTUALMIN_SSH_USERNAME", "virtualmin-praho"),
                "string",
                "SSH username",
            ),
    ...
    def _read_ssh_key_file(self, ssh_key_path: str | None) -> str | None:
        """Read SSH private key from file, return content or None"""
    ...
    def _prepare_credential_metadata(
        self, username_var: str, password_var: str, description: str, ssh_key_content: str | None
    ) -> dict[str, str]:
        """Prepare credential metadata for vault storage"""
        metadata = {
    ...
    def _store_single_credential(self, data: CredentialMigrationData) -> bool:
        """Store a single credential in vault, return success status"""
        username_var, password_var, description = data.credential_info
        try:
            metadata = self._prepare_credential_metadata(username_var, password_var, description, data.ssh_key_content)

            credential_data = CredentialData(
                service_type="virtualmin",
                service_identifier=data.hostname,
                username=data.username,
                password=data.password,
                metadata=metadata,
                reason=f"Migration: {description}",
            )
            result = data.vault.store_credential(credential_data)
            ...
            if result.is_ok():
                ...
                self.stdout.write(
                    self.style.ERROR(f"❌ [Migration] Failed to store {description}: {result.unwrap_err()}")
                )
                return False
    ...
    def _migrate_credentials_to_vault(self) -> None:
        ...
        if not vault:
            self.stdout.write(
                self.style.WARNING("⚠️ [Migration] Credential vault not available, skipping credential migration")
            )
            return
        ...
        for username_var, password_var, description in credentials_to_migrate:
            ...
            if username and password:
                data = CredentialMigrationData(
                    vault=vault,
                    hostname=hostname,
                    credential_info=(username_var, password_var, description),
                    username=username,
                    password=password,
                    ssh_key_content=ssh_key_content,
                )
                if self._store_single_credential(data):
                    migrated_count += 1
            else:
                missing_vars = [var for var, val in [(username_var, username), (password_var, password)] if not val]
    ...
    def _show_cleanup_instructions(self) -> None:
        ...
        env_vars_to_remove = ["VIRTUALMIN_HOST", "VIRTUALMIN_USERNAME", "VIRTUALMIN_PASSWORD", "VIRTUALMIN_SSL_VERIFY"]
