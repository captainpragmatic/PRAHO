...
class TicketStatusService:
    ...
    def handle_first_agent_reply(
        ...
        if ticket.assigned_to is not None:
            logger.warning(f"⚠️ [TicketStatus] Ticket {ticket.ticket_number} already assigned, treating as subsequent reply")
        ...
        logger.info(f"✅ [TicketStatus] First agent reply on {ticket.ticket_number}: {reply_action} -> {new_status}, assigned to {agent}")
        return ticket
    ...
    def handle_customer_reply(cls, ticket: Ticket) -> Ticket:
        ...
        if current_status == "waiting_on_customer":
            if ticket.assigned_to:
                ...
                logger.info(f"📝 [TicketStatus] Customer replied to {ticket.ticket_number}: resuming work (assigned agent)")
            else:
                new_status = "open" 
            ...
            logger.info(f"📝 [TicketStatus] Customer replied to {ticket.ticket_number}: status remains {current_status}")
    ...
    @classmethod
    def _determine_new_status_from_action(
        cls, 
        current_status: str, 
        reply_action: str, 
        default: str
