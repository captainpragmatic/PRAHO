        ...
        if bundle_items.exists() and instance.config.get("bundle_group"):
            ...
            try:
                from apps.provisioning.services import (  # noqa: PLC0415  # Deferred: avoids circular import
                    ServiceGroupService,  # Circular: cross-app signal
                )
    ...
    try:
        ...
        if hasattr(instance.customer, "integrations") and instance.customer.integrations.exists():
            ...
            for integration in active_integrations:
                try:
                    from apps.integrations.services import (  # noqa: PLC0415  # Deferred: avoids circular import
                        ExternalSyncService,  # Circular: cross-app signal
                    )
...
def _update_customer_order_history(order: Order, event_type: str) -> None:
    ...
    try:
        from apps.customers.services import (  # noqa: PLC0415  # Deferred: avoids circular import
            CustomerStatsService,  # Circular: cross-app signal  # Deferred: avoids circular import
        )
...
def _cancel_order_webhooks(order: Order) -> None:
    ...
    try:
        from apps.integrations.models import (  # noqa: PLC0415  # Deferred: avoids circular import
            WebhookDelivery,  # Circular: cross-app signal  # Deferred: avoids circular import
        )
