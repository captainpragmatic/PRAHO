...
def _release_promotions_for_cancelled_order(order: Order) -> None:
    """Delegate cancellation cleanup to the shared trigger-aware composite."""
    try:
        from apps.promotions.services import release_promotions_for_order  # noqa: PLC0415

        release_promotions_for_order(order, trigger="cancellation")
    except Exception:
        logger.error(
            "🔥 [Orders] Promotion release failed for cancelled order %s — "
...
class OrderCalculationService:
    ...
    def calculate_order_totals(
        ...
        from .vat_rules import (  # Circular: same-app  # noqa: PLC0415  # Deferred: avoids circular import
            CustomerVATInfo,
            OrderVATCalculator,
        )
...
class OrderService:
    ...
    def build_billing_address_from_customer(customer: Customer) -> BillingAddressData:
        ...
        from apps.customers.models import (  # noqa: PLC0415  # Deferred: avoids circular import
            CustomerAddress,  # Circular: cross-app  # Deferred: avoids circular import
        )
    ...
    def create_order(data: OrderCreateData, created_by: User | None = None) -> Result[Order, str]:  # noqa: PLR0915
        ...
        try:
            ...
            for item_data in data.items:
                ...
                # Fail if product_id is missing — silently skipping causes total/line-item mismatch (#127)
    ...
    def _create_status_history(
        ...
        from .models import (  # noqa: PLC0415  # Deferred: avoids circular import
            OrderStatusHistory,  # Circular: same-app  # Deferred: avoids circular import
        )
...
class OrderServiceCreationService:
    ...
    def _get_service_plan_for_product(product: Product) -> Result[ServicePlan, str]:
        ...
        try:
            from apps.provisioning.models import (  # noqa: PLC0415  # Deferred: avoids circular import
                ServicePlan,  # Circular: cross-app  # Deferred: avoids circular import
            )
