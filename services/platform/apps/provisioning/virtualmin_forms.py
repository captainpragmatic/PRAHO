...
class VirtualminServerForm(forms.ModelForm):  # type: ignore[type-arg]
    ...
    api_password = forms.CharField(
        widget=PRAHOPasswordWidget(attrs={"placeholder": "Enter API password..."}),
        required=False,
    ...
    class Meta:
        ...
        widgets: ClassVar[dict[str, Any]] = {
            "name": PRAHOTextWidget(attrs={"placeholder": 'Server display name (e.g., "Primary Web Server")'}),
            "hostname": PRAHOTextWidget(attrs={"placeholder": "server.example.com"}),
            "api_port": PRAHOTextWidget(attrs={"placeholder": "10000", "type": "number"}),
            ...
            "max_bandwidth_gb": PRAHOTextWidget(attrs={"placeholder": "5000", "type": "number"}),
        }

    def clean_hostname(self) -> str:
        """Validate hostname format."""
        hostname = cast(str, self.cleaned_data.get("hostname", ""))
    ...
    def clean_api_username(self) -> str:
        ...
        username = cast(str, self.cleaned_data.get("api_username", ""))
...
class VirtualminBackupForm(forms.Form):
    ...
    backup_type = forms.ChoiceField(
        choices=BACKUP_TYPE_CHOICES, initial="full", widget=PRAHOSelectWidget(), help_text=_("Type of backup to create")
    )
    ...
    def clean(self) -> dict[str, Any]:
        ...
        include_email = cast(bool, cleaned_data.get("include_email", False))
        include_databases = cast(bool, cleaned_data.get("include_databases", False))
        include_files = cast(bool, cleaned_data.get("include_files", False))
...
class VirtualminRestoreForm(forms.Form):
    ...
    backup_id = forms.ChoiceField(widget=PRAHOSelectWidget(), help_text=_("Select backup to restore from"))
    ...
    def __init__(self, *args: Any, available_backups: list[dict[str, Any]] | None = None, **kwargs: Any) -> None:
        ...
        if available_backups:
            ...
            cast(forms.ChoiceField, self.fields["backup_id"]).choices = backup_choices
        else:
            cast(forms.ChoiceField, self.fields["backup_id"]).choices = []
    ...
    def clean(self) -> dict[str, Any]:
        ...
        restore_email = cast(bool, cleaned_data.get("restore_email", False))
        restore_databases = cast(bool, cleaned_data.get("restore_databases", False))
        restore_files = cast(bool, cleaned_data.get("restore_files", False))
...
class VirtualminAccountForm(forms.ModelForm):  # type: ignore[type-arg]
    ...
    def clean_domain(self) -> str:
        ...
        domain = cast(str, self.cleaned_data.get("domain", "")).strip().lower()
