...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: Any) -> None:
        ...
        parser.add_argument("--server-id", type=str, help="Test specific server by UUID")
        ...
        parser.add_argument(
            "--fix-failures", action="store_true", help="Attempt to fix authentication failures automatically"
        )

    def handle(self, *args: Any, **options: Any) -> None:
        """Execute the authentication health test."""

        self.stdout.write(self.style.SUCCESS("🔐 Testing Virtualmin Authentication Health..."))
    ...
    def _test_single_server(self, server_id: str, options: dict[str, Any]) -> None:
    ...
    def _test_all_servers(self, options: dict[str, Any]) -> None:
        ...
        for server_detail in health_summary["server_details"]:
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"❌ Server {server_detail['server_id']} not found in database"))
                continue

    def _display_summary(self, health_summary: dict[str, Any]) -> None:
        ...
        self.stdout.write(status_style(f"{status_emoji} Overall ACL Health: {healthy_percentage:.1f}%"))

        self.stdout.write(f"📈 Servers tested: {total}")
        self.stdout.write(self.style.SUCCESS(f"✅ ACL working: {acl_working}"))

        if acl_failed > 0:
            self.stdout.write(self.style.ERROR(f"❌ ACL failed: {acl_failed}"))

        if fallback_working > 0:
            self.stdout.write(self.style.WARNING(f"🔄 Fallback available: {fallback_working}"))

        if completely_failed > 0:
            self.stdout.write(self.style.ERROR(f"🔥 Complete failures: {completely_failed}"))

        # Risk assessment
        if acl_failed > 0:
            self.stdout.write("\n🚨 ACL AUTHENTICATION RISK DETECTED!")
            self.stdout.write(self.style.ERROR(f"   {acl_failed} servers have failed ACL authentication."))
    ...
    def _display_server_results(
        ...
        for method_name, result in health_results.items():
            ...
            if result.success:
                ...
                self.stdout.write(self.style.SUCCESS(f"   ✅ {method_name}: Working{response_time}"))
            else:
                self.stdout.write(self.style.ERROR(f"   ❌ {method_name}: {result.error}"))

        # Overall server assessment
        working_methods = sum(1 for result in health_results.values() if result.success)

        if working_methods == 0:
            self.stdout.write(self.style.ERROR("   🔥 CRITICAL: No authentication methods working!"))
        elif not health_results.get("acl", {}).success:
            self.stdout.write(self.style.WARNING("   ⚠️  ACL authentication failed - using fallback"))
        else:
            self.stdout.write(self.style.SUCCESS("   🎯 All authentication methods healthy"))

    def _attempt_fix(self, server: VirtualminServer, server_detail: dict[str, Any]) -> None:
        ...
        self.stdout.write(self.style.WARNING("   Fix mode not yet implemented - would:"))
