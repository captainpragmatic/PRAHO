        ...
        for account in accounts:
            try:
                ...
                if domain_info_result.is_ok():
                    ...
                    self.stdout.write(self.style.ERROR(f"❌ {account.domain}: {error_msg}"))
                    error_count += 1

            except Exception as e:
                self.stdout.write(self.style.ERROR(f"❌ {account.domain}: Exception - {e!s}"))
                error_count += 1

        if not dry_run:
            self.stdout.write(self.style.SUCCESS(f"Sync completed: {updated_count} updated, {error_count} errors"))
