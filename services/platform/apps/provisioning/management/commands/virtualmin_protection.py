...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: Any) -> None:
        ...
        parser.add_argument("--enable", action="store_true", help="Enable deletion protection")

        parser.add_argument("--disable", action="store_true", help="Disable deletion protection")

        parser.add_argument("--status", action="store_true", help="Show protection status for all accounts")

        parser.add_argument("--all", action="store_true", help="Apply to all accounts")

        parser.add_argument("--domain", type=str, help="Apply to specific domain")
    ...
    def _show_status(self) -> None:
        ...
        for account in accounts:
            ...
            self.stdout.write(f"{status_icon} {account.domain:<30} {status_text:<12} [{account.status}]")
    ...
    def _enable_protection(self, options: dict[str, Any]) -> None:
        ...
        self.stdout.write(self.style.SUCCESS(f"✅ Enabled deletion protection for {updated_count} account(s)"))
    ...
    def _disable_protection(self, options: dict[str, Any]) -> None:
        ...
        self.stdout.write(self.style.WARNING(f"⚠️  Disabled deletion protection for {updated_count} account(s)"))
