...
class ComprehensiveAccessControlTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.customer_user = User.objects.create_user(
            email="customer@example.com", password="customerpass123", first_name="Customer", last_name="User"
        )

        # Create customer organization
        self.customer = Customer.objects.create(
            name="Test Customer", company_name="Test Company Ltd", primary_email="customer@example.com"
        )
        ...
        self.other_customer = Customer.objects.create(
            name="Other Customer", company_name="Other Company Ltd", primary_email="other@example.com"
        )
        ...
        self.service_plan = ServicePlan.objects.create(
            name="Basic Hosting", plan_type="shared_hosting", price_monthly=Decimal("50.00"), is_active=True
        )
    ...
    def test_payment_processing_restricted_to_staff(self):
        ...
        response = self.client.post(url, {"amount": "100.00", "payment_method": "bank_transfer"})
    ...
    def test_internal_ticket_notes_restricted_to_staff(self):
        ...
        response = self.client.post(url, {"reply": "This is an internal note", "reply_action": "internal_note"})

        # Should be redirected back to ticket with error message
        self.assertEqual(response.status_code, 302)

        # Verify no internal note was created
        internal_comments = TicketComment.objects.filter(ticket=self.ticket, comment_type="internal")
    ...
    def test_staff_can_create_internal_ticket_notes(self):
        ...
        url = reverse("tickets:reply", args=[self.ticket.pk])
        response = self.client.post(url, {"reply": "This is a staff internal note", "reply_action": "internal_note"})
...
class SecurityDecoratorsTestCase(TestCase):
    ...
    def setUp(self):
        """Set up test users"""
        self.staff_user = User.objects.create_user(
            email="staff@praho.ro", password="staffpass123", is_staff=True, staff_role="admin"
        )

        self.customer_user = User.objects.create_user(email="customer@example.com", password="customerpass123")
