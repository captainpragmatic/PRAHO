...
class GDPRExportServiceTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            email="test@example.com", password="testpass123", first_name="Ion", last_name="Popescu"
        )
    ...
    def test_create_data_export_request_success(self):
        ...
        export_scope = {"include_profile": True, "include_customers": True, "include_billing": False, "format": "json"}

        result = gdpr_export_service.create_data_export_request(
            user=self.user, request_ip="192.168.1.1", export_scope=export_scope
        )
    ...
    def test_create_data_export_request_duplicate_prevention(self):
        ...
        result1 = gdpr_export_service.create_data_export_request(user=self.user, export_scope=export_scope)
        self.assertTrue(result1.is_ok())

        # Create second request (should also succeed - no duplicate prevention implemented)
        result2 = gdpr_export_service.create_data_export_request(user=self.user, export_scope=export_scope)
    ...
    def test_process_data_export_success(self):
        ...
        result = gdpr_export_service.create_data_export_request(
            user=self.user, export_scope={"include_profile": True, "format": "json"}
        )
        export_request = result.value
...
class GDPRDeletionServiceTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            email="delete@example.com", password="testpass123", first_name="Maria", last_name="Ionescu"
        )
    ...
    def test_create_deletion_request_anonymize(self):
        ...
        result = gdpr_deletion_service.create_deletion_request(
            user=self.user, deletion_type="anonymize", request_ip="10.0.0.1", reason="No longer need account"
        )

        self.assertTrue(result.is_ok())

        # Check compliance log entry
        log_entry = ComplianceLog.objects.filter(compliance_type="gdpr_deletion", user=self.user).first()
    ...
    def test_create_deletion_request_delete(self):
        ...
        result = gdpr_deletion_service.create_deletion_request(
            user=self.user, deletion_type="delete", request_ip="10.0.0.1", reason="Complete data removal requested"
        )
    ...
    def test_process_deletion_request_anonymize(self):
        ...
        result = gdpr_deletion_service.create_deletion_request(
            user=self.user, deletion_type="anonymize", reason="Privacy request"
        )
...
class GDPRConsentServiceTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email="consent@example.com", password="testpass123")
    ...
    def test_withdraw_consent_marketing_only(self):
        ...
        result = gdpr_consent_service.withdraw_consent(
            user=self.user, consent_types=["marketing"], request_ip="192.168.1.100"
        )
        ...
        log_entry = ComplianceLog.objects.filter(
            compliance_type="gdpr_consent", user=self.user, description__icontains="marketing"
        ).first()
    ...
    def test_withdraw_consent_data_processing(self):
        ...
        result = gdpr_consent_service.withdraw_consent(
            user=self.user, consent_types=["data_processing"], request_ip="192.168.1.100"
        )
        ...
        consent_log = ComplianceLog.objects.filter(compliance_type="gdpr_consent", user=self.user).first()
        self.assertIsNotNone(consent_log)

        deletion_log = ComplianceLog.objects.filter(compliance_type="gdpr_deletion", user=self.user).first()
...
class GDPRSecurityTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email="security@example.com", password="testpass123")
...
class DataExportOrphanedRequesterTestCase(TestCase):
    """W7: deleting a user must not erase (CASCADE) or crash the GDPR export trail."""

    def _export(self, **overrides):
        user = User.objects.create_user(email=f"orphan{DataExport.objects.count()}@example.com", password="x")
        fields = {
        ...
        return DataExport.objects.create(**fields)

    def test_user_deletion_preserves_the_export_record(self) -> None:
        export = self._export(status="completed")
        export.requested_by.delete()
        export.refresh_from_db()
        self.assertIsNone(export.requested_by)
        self.assertEqual(export.requested_by_display, "deleted user")
        self.assertIn("deleted user", str(export))

    def test_orphaned_pending_export_transitions_to_failed(self) -> None:
        from apps.audit.services import gdpr_export_service  # noqa: PLC0415  # Deferred: test-local

        export = self._export(status="pending")
        export.requested_by.delete()
        export.refresh_from_db()

        result = gdpr_export_service.process_data_export(export)

        self.assertTrue(result.is_err())
        export.refresh_from_db()
        self.assertEqual(export.status, "failed")
        self.assertIn("deleted", export.error_message)
