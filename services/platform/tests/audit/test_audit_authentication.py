...
class AuthenticationAuditServiceTest(TestCase):
    ...
    def test_log_login_success_basic(self):
        ...
        event_data = AuthenticationEventData(user=self.user, request=self.request, authentication_method="password")
    ...
    def test_log_login_failed_user_not_found(self):
        ...
        failure_data = LoginFailureEventData(
            email="nonexistent@example.com", failure_reason="user_not_found", request=self.request
        )
    ...
    def test_log_login_failed_invalid_password(self):
        ...
        failure_data = LoginFailureEventData(
            email="test@example.com", user=self.user, failure_reason="invalid_password", request=self.request
        )
        audit_event = AuthenticationAuditService.log_login_failed(failure_data)
    ...
    def test_log_login_failed_account_locked(self):
        ...
        with patch.object(self.user, "is_account_locked", return_value=True):
            failure_data = LoginFailureEventData(
                email="test@example.com", user=self.user, failure_reason="account_locked", request=self.request
            )
            audit_event = AuthenticationAuditService.log_login_failed(failure_data)
    ...
    def test_log_logout_manual(self):
        ...
        logout_data = LogoutEventData(user=self.user, logout_reason="manual", request=self.request)
    ...
    def test_log_logout_session_expired(self):
        ...
        logout_data = LogoutEventData(user=self.user, logout_reason="session_expired", request=self.request)
        audit_event = AuthenticationAuditService.log_logout(logout_data)
    ...
    def test_log_account_locked(self):
        ...
        account_data = AccountEventData(
            user=self.user, trigger_reason="excessive_failed_attempts", request=self.request, failed_attempts=5
        )
    ...
    def test_metadata_serialization_safety(self):
        ...
        event_data = AuthenticationEventData(user=self.user, request=self.request, metadata=complex_metadata)
        audit_event = AuthenticationAuditService.log_login_success(event_data)
...
class AuthenticationSignalsTest(TestCase):
    ...
    def test_signal_exception_handling(self):
        ...
        with patch(
            "apps.users.signals.AuthenticationAuditService.log_login_success",
...
class AuthenticationViewsIntegrationTest(TestCase):
    ...
    def test_login_view_creates_audit_event(self):
        ...
        response = self.client.post(reverse("users:login"), {"email": "view@example.com", "password": "testpass123"})
    ...
    def test_logout_view_creates_audit_event(self):
        ...
        view_triggered_events = audit_events.filter(metadata__logout_triggered_by="logout_view")
    ...
    def test_failed_login_creates_audit_event(self):
        ...
        response = self.client.post(reverse("users:login"), {"email": "view@example.com", "password": "wrongpassword"})
...
class AuthenticationAuditQueryPerformanceTest(TestCase):
    ...
    def test_audit_indexes_exist(self):
        ...
        from django.db import connection  # noqa: PLC0415  # Deferred: test-local backend introspection
        ...
        for expected_index in expected_indexes:
            self.assertIn(expected_index, indexes, f"Missing authentication audit index: {expected_index}")
    ...
    def test_authentication_audit_query_efficiency(self):
        ...
        for i in range(10):
            event_data = AuthenticationEventData(
                user=self.user, ip_address=f"192.168.1.{i}", authentication_method="password"
            )
            AuthenticationAuditService.log_login_success(event_data)

        # Test user-based query (should use idx_audit_user_action_time)
        with self.assertNumQueries(1):
            events = list(AuditEvent.objects.filter(user=self.user, action="login_success").order_by("-timestamp")[:5])
            self.assertEqual(len(events), 5)

        # Test IP-based query (should use idx_audit_ip_action_time)
        with self.assertNumQueries(1):
            events = list(
                AuditEvent.objects.filter(ip_address="192.168.1.5", action="login_success").order_by("-timestamp")
            )
...
class AuthenticationAuditGDPRComplianceTest(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.user = User.objects.create_user(email="gdpr@example.com", password="testpass123")

    def test_audit_event_contains_required_gdpr_fields(self):
        """Test that audit events contain required GDPR fields"""
        event_data = AuthenticationEventData(user=self.user, ip_address="192.168.1.1")
        audit_event = AuthenticationAuditService.log_login_success(event_data)
    ...
    def test_audit_event_immutability(self):
        ...
        event_data2 = AuthenticationEventData(user=self.user, ip_address="192.168.1.2")
