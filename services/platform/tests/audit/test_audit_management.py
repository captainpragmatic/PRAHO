...
class EnterpriseAuditManagementTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.staff_user = User.objects.create_user(
            email="staff@example.com", password="testpass123", is_staff=True, staff_role="admin"
        )
        ...
        self.regular_user = User.objects.create_user(email="user@example.com", password="testpass123")
...
class AuditIntegrityMonitoringTests(EnterpriseAuditManagementTestCase):
    ...
    def test_integrity_check_hash_verification(self):
        ...
        result = audit_integrity_service.verify_audit_integrity(
            period_start=start_time, period_end=end_time, check_type="hash_verification"
        )
    ...
    def test_integrity_check_sequence_gaps(self):
        ...
        result = audit_integrity_service.verify_audit_integrity(
            period_start=start_time, period_end=end_time, check_type="sequence_check"
        )
    ...
    def test_integrity_check_gdpr_compliance(self):
        ...
        result = audit_integrity_service.verify_audit_integrity(
            period_start=start_time, period_end=end_time, check_type="gdpr_compliance"
        )
    ...
    def test_manual_integrity_check_trigger(self):
        ...
        self.assertTrue(AuditIntegrityCheck.objects.filter(check_type="hash_verification").exists())
...
class AuditRetentionManagementTests(EnterpriseAuditManagementTestCase):
    ...
    def test_retention_policy_application_confirmation(self):
        ...
        # With confirmation
        response = self.client.post(reverse("audit:apply_retention_policies"), {"confirm": "yes"})
        self.assertEqual(response.status_code, 302)
    ...
    def test_financial_record_protection(self):
        ...
        # Apply policies
        audit_retention_service.apply_retention_policies()
...
class AuditAlertsManagementTests(EnterpriseAuditManagementTestCase):
    ...
    def test_alert_status_updates(self):
        ...
        response = self.client.post(
            reverse("audit:update_alert_status", args=[self.alert.id]), {"action": "acknowledge"}
        )
        self.assertEqual(response.status_code, 302)
        ...
        # Assign to self
        response = self.client.post(
            reverse("audit:update_alert_status", args=[self.alert.id]), {"action": "assign_to_me"}
        )
        self.assertEqual(response.status_code, 302)
...
class AuditExportEnhancementsTests(EnterpriseAuditManagementTestCase):
    ...
    def test_json_export_with_filters(self):
        ...
        export_params = {"format": "json", "category": ["security_event"], "severity": ["critical"]}
...
class SecurityAndPerformanceTests(EnterpriseAuditManagementTestCase):
    ...
    def test_search_query_permissions(self):
        ...
        other_staff = User.objects.create_user(email="other_staff@example.com", password="testpass123", is_staff=True)

        private_query = AuditSearchQuery.objects.create(
            name="Private Query", query_params={"category": "authentication"}, created_by=other_staff, is_shared=False
        )
    ...
    def test_audit_event_immutability(self):
        """Audit events cannot be modified after creation - the guard raises, not a docstring."""
        from apps.audit.models import AuditImmutabilityError  # noqa: PLC0415
        ...
        self.assertNotEqual(event.description, "Modified description")
