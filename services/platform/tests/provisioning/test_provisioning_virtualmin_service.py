...
class VirtualminProvisioningServiceTest(TestCase):
    ...
    def test_server_connection_test(self, mock_gateway_class):
        """Test server connection testing with MockVirtualminGateway"""
        ...
        # MockVirtualminGateway.test_connection() returns {"healthy": ..., "server": ..., ...}
        self.assertTrue(connection_info["healthy"])
        self.assertEqual(connection_info["server"], "mock-server.example.com")
...
class VirtualminServerManagementServiceTest(TestCase):
    ...
    def test_health_check_success(self, mock_gateway_class):
        """Test successful server health check using MockVirtualminGateway"""
        mock_gw = MockVirtualminGateway()
        mock_gateway_class.return_value = mock_gw
    ...
    def test_statistics_update(self, mock_gateway_class):
        """Test server statistics update using MockVirtualminGateway"""
        mock_gw = MockVirtualminGateway()
        # Seed domains so list_domains returns realistic data
        mock_gw.seed_domain("example1.com")
        mock_gw.seed_domain("example2.com")
        mock_gw.seed_domain("example3.com")
        mock_gateway_class.return_value = mock_gw
