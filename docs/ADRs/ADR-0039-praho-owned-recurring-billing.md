...
- The global collection switch and customer row form one ordering boundary shared by final revalidation, gateway submission, mandate changes, per-service enrollment, cancellation, and renewal opt-out. The customer lock uses PostgreSQL `NO KEY UPDATE` so unrelated foreign-key inserts, including manual Payments, cannot invert the customer/document lock order.
- If a disabling mutation acquires that boundary first, the reserved attempt is abandoned before Stripe. If submission acquires it first, the attempt is already in flight; the mutation waits for the gateway request to return and does not manufacture an automatic refund.
...
- GitHub issues #209, #218, #219, and #301
