...
- **Discounted billing parity** — document discounts now reduce the taxable base before VAT with deterministic per-category allocation and rounding; order preflight, proformas, invoices, PDFs, and e-Factura UBL share the same reconciled ledger, while card collection and fallback conversion use the frozen proforma total instead of a mutable order total (#195, #203)
