...
- **Multi-label Romanian TLD extraction** — `DomainValidationService.extract_tld_from_domain` (and `Domain.clean()`, which now shares it) resolved the TLD with a bare `rsplit('.', 1)`, so `shop.com.ro` became `.ro` — mislinking/mispricing the domain or falsely rejecting it as unsupported; extraction now matches the longest configured `TLD.extension` suffix, so `.com.ro`/`.org.ro` resolve correctly while single-label TLDs are unaffected (#237). (The latent renewal/registration defects bundled in #237 are not addressed here — those paths have no live caller yet.)
