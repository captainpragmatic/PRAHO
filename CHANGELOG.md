...
- **Honest asynchronous registrar operations with durable reconciliation** — Gandi answers chargeable mutations with `202 Accepted` plus a `Location` operation handle, which the adapter previously either fabricated into completion (lock, nameservers) or failed as an invalid response (register, renew — every real registration would have errored). Accepted-but-unconfirmed outcomes are now first-class: the gateway returns pending results carrying the operation handle, services record a `DomainOperation` in `submitted` state instead of lying locally, and a new hourly reconciliation worker converges them through the registrar's read API — activating confirmed registrations and transfers, completing renewals only when the expiry provably advanced past the recorded pre-renewal value, applying nameserver/lock changes only when the registrar's state matches the request, deleting a pending registration only after a definite not-found persisting past 24 hours (never for in-flight transfers), and failing anything unconfirmed after 72 hours with visibility instead of guesses. Renewal and availability calls now carry the reseller `sharing_id` (previously the chargeable renewal billed the PAT's default organization) and transfer-in sends the owner contact Gandi requires, validated before any local row is created. The staff renewal page reports an accepted-but-unconfirmed renewal as awaiting confirmation rather than completed. Active domains strictly past their expiry plus the TLD grace window are finally expired locally — inside grace they stay active so grace renewals keep working (#257, #258)
- **Paid orders actually provision their domain items** — the domain order pipeline (validated items, ownership re-checks, per-item idempotency tokens) existed without a caller, so a paid order containing domain registrations or renewals never touched the registrar. The transition into provisioning now enqueues an async task on commit, keeping registrar network I/O out of the order transaction; every order item is processed behind one per-item exception boundary so an infrastructure failure on one item cannot abort the rest, and a second legitimate staff renewal within the idempotency window is no longer silently collapsed into the first (each rendered renewal form carries its own durable intent token) (#259, #465)
- **Internationalized domain names canonicalize through IDNA UTS-46** — a Unicode U-label and its punycode A-label previously could coexist as distinct rows when a writer bypassed service validation; the single canonicalization choke point now folds non-ASCII names to the A-label (never raising, with a data migration for legacy rows that refuses loudly on collisions) (#473)
- **Automated BNR exchange-rate ingestion (opt-in)** — a feature-flagged daily task fetches BNR reference rates through an SSRF-safe, batch-validated gateway and records them via a shared immutable writer (never overwriting a differing rate or provenance); any fetch/parse failure retains last-good rates and alerts staff. Rates are stamped with the correct statutory validity date (publication + 1 calendar day, art. 290(2)), and issued invoices freeze their RON exchange rate at conversion so a later row can never change a filed VAT total. A resolver-accurate FX-freshness card on the system-status dashboard shows GREEN/AMBER/RED per foreign currency, with daily staleness alerts (#103, ADR-0046).

### Fixed

- **The checkout page shows the payment-card icons again** — the Visa and Mastercard logos beside the payment step had never been part of the repository. A catch-all rule that keeps throwaway screenshots out of version control could not tell them from a hand-authored image and quietly excluded them, so the page rendered two broken images at the moment customers are asked to pay, and nothing anywhere reported a problem. The icons are restored, that rule now makes an exception for images the site actually uses, and a check fails the build if any page ever again refers to an image the repository does not contain.

- **Refunds now record who issued them** — every refund the platform has ever written was stored with no author. The service read the acting user from a key that nothing sets, while the three refund paths all supplied it under a different name, so the audit column was populated on no path and by no route; the refund's own opening status-history row recorded no author either, and the billing audit trail attributes every refund to "system" regardless of who clicked. Staff and portal refunds now name the person who initiated them on both the refund and its first history entry, and the portal refund API carries through the account member it had already verified instead of discarding the identity after the permission check. A gateway-initiated refund still records no person, which is correct — no human issued it.

- **Refund reasons are no longer silently discarded** — the refund forms offered eleven reasons of which only three were values the refund record could actually hold, and because the database is not asked to validate the column, the other eight were written and kept as-is: a refund marked "Quality Not As Expected" or "Duplicate Invoice" carried a reason no report or filter could match. The refund record now recognises the full vocabulary the forms offer, the handful of reasons that existed under two different spellings are unified onto one (the wording on the forms is unchanged), and refunds already stored under the old spellings are migrated. Refund types are normalised the same way, including for refunds still in flight at the time of the upgrade, so a retried request cannot fail to recognise the refund it already started and issue a second one. Refunds made through the customer portal, which could previously record a free-text reason, are covered by the same vocabulary.

- **e-Factura audit events are actually recorded** — none of the e-Factura audit calls had ever written an audit event. Each built its event with a field the audit system does not accept, which raised an error immediately, and every call site caught and logged that error as a warning and carried on, so XML generation, submission attempts, ANAF acceptances and rejections, status changes, retries and downloads all appeared to be audited while nothing was written. The accompanying tests could not catch it because they replaced the audit types with stand-ins that accept anything. The event details, including the severity of a failed submission or an ANAF rejection, now reach the audit trail, and the tests exercise the real types.

- **Refunds require financial authority, not merely a staff account** — issuing a refund was gated on being staff at all, so a support agent could refund any invoice or order for any customer in full; the per-document access check offered no second line of defence because it admits every staff user by design. Refunds on both the invoice and order paths now require the admin, billing or manager role that the platform already reserves for financial operations, the controls are hidden from staff who lack it rather than failing after the click, and the portal refund API additionally requires an owner or billing principal instead of accepting any customer member including a read-only viewer. Denials are returned as JSON, and both refund dialogs now read whichever field carries the reason, so every failure they can hit — an authorization refusal, an already-refunded invoice, an invalid amount — is reported to the user instead of the literal text "Error: true" (#104).
- **Database distress during order numbering surfaces instead of degrading silently** — the row-lock fallback protecting order numbers treated any database error as an unsupported backend and quietly retried without the lock, hiding deadlocks, lock timeouts and broken connections behind a race on the number it was protecting. Only genuinely unsupported backends now take that fallback. The matching fallback for order totals deliberately keeps its broader handling, because its only caller discards exceptions: surfacing there would leave a saved order item missing from its own total rather than making anything visible (#104).

- **Deployment-specific values no longer assume a single operator** — VAT decisions compare the customer's country against the operator's own jurisdiction rather than a hardcoded Romania, so a deployment established elsewhere no longer zero-rates its domestic business sales while charging Romanian VAT to Romanian customers. Audit retention periods derived from national law install as mandatory only where the operator is actually established, instead of silently deleting audit records at the Romanian boundary everywhere; the privacy periods no longer cite a GDPR article that prescribes no retention. Large-refund alerts send to a configured finance address and are suppressed when none is set, rather than mailing invoice and customer data to a fixed external mailbox, and the alert now states the threshold that actually fired. Romanian behaviour is unchanged throughout (#517, #518, #519).
- **Out-of-scope invoices are valid electronic documents** — supplies outside the scope of VAT no longer carry a zero rate or party VAT identifiers on the line, allowance, seller and buyer elements, which the European e-invoicing standard forbids; the invoice and credit-note paths are both corrected, and the local validator now detects the violation instead of passing it (#523).
- Repaired customer account, checkout, billing, service and support workflows exposed by the E2E audit: persisted signup/order identity, user-scoped MFA and password changes, verified company switching, complete invoice synchronization and service search, usable product status toggles, ticket attachment upload/download, and mobile layouts. Manual proforma payment controls now follow locked service eligibility and billing-role authorization; generated sample data and audit events use current fiscal/model contracts.
- Restored default Django discovery for API and query-performance tests, with a guard against silently omitted test directories.

- **Foreign-currency billing is now safe and honored** — a staff-selected proforma currency (EUR/USD) is persisted instead of being silently forced to RON, and every foreign-currency billing path (staff/order proforma, subscription, recurring, and checkout) fails closed when no provenanced BNR/ECB exchange rate is available, so a payment can never be taken for a document that could not be legally issued as an invoice. `BILLING_DEFAULT_CURRENCY` is validated by a startup system check (#103).
- **Registrar adapters follow documented contracts and retain uncertain operations** — Gandi 202 responses remain pending until domain state confirms completion; ROTLD uses historical REST v2 Digest/form commands with persisted contact IDs and separate nameserver setup. Durable renewal intents prevent duplicate submissions after cache loss, expiry evidence cannot confirm two renewals, and unresolved operations remain available for review instead of being deleted or timed out as failures. Offline contract and PostgreSQL concurrency tests cover the flow; live validation is deferred and the adapter verification gate remains off (#257).
- **Dev servers no longer melt down under sustained API traffic** — django-debug-toolbar 6.2.0's cache panel stacked one wrapper per instrumented request per thread (its re-patch guard keyed on an attribute that request teardown reset), so the portal's keep-alive session to the platform grew a wrapper tower on one server thread until any cache read — including every login's settings lookup — died with RecursionError at ~933 frames, and the e2e suite collapsed mid-run with hundreds of "Login failed" errors. The dependency floor now encodes the fixed version (≥6.3.0, locked at 8.0.0), a regression test pins wrapper depth at exactly one across 50 instrumentation cycles, and the toolbar callback on both services now honors `INTERNAL_IPS` and skips `/api/` requests it can never render into. Three stale platform e2e specs (tab-widget filter, DNS-zone deployment gate, staff-user detail targeting) were repaired alongside, and dev sample data now seeds a deployment DNS zone so the create page is reachable out of the box (#494).
...
- **Cancelled orders release their coupons, against the campaign that was actually charged** — cancelling an order (staff action, review rejection, payment timeout, or invoice void) never reversed its coupon redemption: a single-use coupon was permanently consumed by an order that was never paid, and the campaign budget stayed charged for a discount never given. Every cancellation path now funnels the reversal through the order-status service inside the cancel transaction — deliberately not the post-commit signal chain, whose blanket exception handling would have reduced a failed money-reversal to a log line — and a promotions failure cannot block the cancellation itself (the coupon stays conservatively consumed, with a loud log and audit event for manual follow-up). Underneath it, the reversal now decrements the campaign that was *charged*: the coupon's campaign is admin-editable, so reversing via its current value could leave the original campaign charged forever while silently driving the new one's spend negative (no database constraint catches it); each redemption snapshots the charged campaign at application time and migration 0003 backfills applied rows (#481, #482)

- **Domain audit events reflect real changes** — every domain update used to emit spurious lock-changed and WHOIS-privacy-changed security audit events because original values were stored as strings while comparisons used booleans; the TLD sibling flagged every price-neutral update as a pricing change and then crashed the change calculator. Originals are stored with native types, and the unreachable transfer-completion signal branch keyed on statuses outside the domain FSM is gone (#465)

- **Audit review queue for flagged events** — `AuditEvent.requires_review` was set automatically for high-risk actions but was write-only: no way to list flagged events, mark one reviewed, or record who did it. Staff now get a filterable, paginated queue at `/audit/review/` (awaiting-review by default, filterable by severity and reviewed state) with a mark-reviewed action that records reviewer, timestamp, and optional notes, and is itself audited. Review state lives in a new `AuditEventReview` companion row rather than columns on `AuditEvent`, so the append-only evidence invariant holds — marking an event reviewed is a routine user-driven action, and putting it on `AuditEvent` would have meant entering the `audit_mutation_allowed()` escape hatch on every click, alongside GDPR erasure and retention deletion, in fields the integrity MAC does not cover. A concurrent double-submit keeps the first review rather than erroring on the one-to-one or silently overwriting. Deletion cascades from the event, not the annotation: `audit_event` is `CASCADE` and `reviewed_by` is `SET_NULL`, because `PROTECT` would let a review veto the deletion of the very event it annotates — retention deletion and GDPR erasure would raise `ProtectedError` as soon as one reviewed event fell in scope, making an erasure request unfulfillable, and a departing staff account would be undeletable. The audit management dashboard's four stat cards also rendered a permanent `0` — the view supplied its counts under `audit_stats` while the template read flat names that were never set — so the flagged-event count this queue is reached from was invisible; the cards are now bound to the real numbers and link to the queue (#400)
- **Encryption key rotation can now actually complete** — with `ENCRYPTION_KEYS=[new, old]`, a v2 row still encrypted under `old` decrypts fine through keyring fallback, so `reencrypt_with_aad` skipped it as "already v2" and the superseded key could never be retired: the rotation ran clean and silently never finished. `--rekey` re-encrypts exactly those rows under the current key, preserving each row's AAD binding, and is idempotent. A plain run now states that already-v2 rows were *not* checked against the current key, so a clean run can no longer be mistaken for permission to drop a key. Backed by a new `decrypts_under_current_key()` helper, since `decrypt_sensitive_data` deliberately reports success without revealing which key worked. Retirement guidance is explicit that `0 rekeyed` is necessary but not sufficient: it describes the rows the scan visited when it visited them, so a writer still on the previous configuration — mid rolling deploy, a stale worker — has the old key first in its own keyring and encrypts new writes under it, including to rows already scanned; retiring on that basis makes those rows permanently undecryptable. The safe order is rollout first, then `--rekey` to `0`, then retire, and a clean `--rekey` run prints that caveat where the decision is actually made. The helper also honours its "never raises" contract against a non-`str` column value and a malformed keyring, either of which previously aborted a sweep partway (#270)

- **Automated node DNS provisioning** — deploying a node now creates its own Cloudflare A (and AAAA) records during the `configuring_dns` stage, before panel installation, and removes them on every teardown path (interactive destroy, the immediate failed-deploy compensator, and the scheduled cleanup sweep). Records are tagged to the owning deployment so an operator's manual records are never touched, replay duplicates converge to one, and a partially-created record is never leaked; a missing DNS token/zone fails before any paid server is created, and an unauthorized or wrong zone fails closed with the just-provisioned server compensated. Making the panel's Let's Encrypt certificate issuance fatal is tracked separately (#347, #436)
- **Staff API-token management** — staff can list, create, and revoke only their own active API tokens from Settings; raw credentials are displayed once on a non-cacheable response, while token descriptions now round-trip through both issuance and identity API endpoints (#244, #247)
...
- **Keyed audit integrity (v2) with no downgrade path** — every audit event is stamped with an HMAC-SHA256 over the evidence itself (action, actor, object identity, old/new values, metadata) keyed via a dedicated derivation domain with key-id rotation; after the batched `restamp_audit_integrity` cutover a missing or downgraded marker verifies as compromised, GDPR anonymization no longer raises false criticals, and a verification that crashes persists an error-status check row and exits non-zero instead of defaulting to healthy (#217, #313)
- **Enforced audit immutability** — AuditEvent mutation through any ORM path raises unless the call site enters the `audit_mutation_allowed()` escape hatch with an auditable reason; every legitimate mutator (GDPR erasure/anonymization, retention, integrity stamping, reset cleanup) is enumerated and tested, and ContentType deletion can no longer cascade audit evidence away
- **Audit retention that actually runs** — one policy-driven engine with seeded per-category policies (10-year mandatory financial retention, 5-year mandatory GDPR-accountability anonymization), DB-enforced mandatory-policy semantics, an effective financial cutoff, an allowlist-based anonymization invariant that re-stamps scrubbed rows, weekly scheduling, and durable file-integrity baselines that survive cache flushes with an explicit `--rebaseline` deploy step
...
- **Canonical e-Factura submission** — removed the duplicate legacy submission engine and debug simulation path; generic billing tasks and compatibility callers now use the audited `EFacturaDocument` lifecycle, enqueue exactly once after the invoice transaction commits, and propagate submission failures instead of reporting false task success
- **Unsafe plan-change engine retired** — historical `SubscriptionChange` records remain readable, but the unwired executable path was removed because it changed billing without coordinating provisioning, never applied scheduled changes, and discarded downgrade credits; normal subscription renewal is unchanged

### Fixed

- **Order-driven domain renewals no longer collapse distinct intents** — the renewal idempotency key was `{domain}:{years}`, so a legitimate second one-year renewal of the same domain inside the cache TTL replayed the first cached result instead of contacting the registrar: the customer is billed for a renewal that never happened and the expiry never moves. `renew_domain` now accepts an optional `idempotency_token` naming one intent, and the order path passes the `DomainOrderItem` pk — durable, exactly one per item, so a retried batch still replays rather than re-charging while two separate order items stay distinct. Callers with no durable identifier (the web renewal view) keep the previous key shape deliberately: minting a fresh token per call would remove double-submit protection entirely, which is the worse failure at a registrar that charges per renewal. That path remains tracked in #259.
- **Domain-order renew items are processable and unhandled actions are logged** — `create_domain_order_item` never linked the existing `Domain` onto a `renew` item, so `process_domain_order_items`' `elif item.action == "renew" and item.domain` guard was never true and every renewal was silently skipped; renew items now link the customer's owned `Domain` when it exists, and the processing loop logs loudly for an unlinked renew, warns that order-driven `transfer` items are not auto-processed yet (rather than dropping them), and logs any unhandled action instead of dropping it silently (#430, the last latent finding from #237). Renewal ownership is re-checked at processing time, since `DomainOrderItem` rows can be created outside `create_domain_order_item`. These paths have no live caller yet — this is a fix-before-wiring change; per-item idempotency and `Domain.name` canonicalization are tracked in #442.
...
- **One visibility predicate for ticket comments across web, template, and API** — comment visibility was decided three different ways: the web views filtered `comment_type__in=("customer","support")`, the API filtered `is_public`, and the template gated on `comment_type != "internal"`. The two columns are independent with nothing tying them, so the predicates disagreed in both directions — a `support` comment with `is_public=False` was **shown to the customer by the web path** while the API hid it, and a public `system` comment was hidden by the web path while the API showed it. All surfaces now derive from `is_public` through a single `TicketComment.visible_to()` helper, attachment downloads follow their parent comment's visibility instead of a separate `comment_type` check, and the customer-facing list API no longer reports a comment count that includes staff-only internal notes (now a filtered annotation, so a ticket list stays one query). The sample-data generator paired a round-robin `comment_type` with a hardcoded `is_public=True`, seeding exactly these contradictory rows into every demo database; it now derives one from the other (#278)
- **`Domain.name` is stored canonically** — DNS names are case-insensitive, but `name` is a plain `CharField` with `unique=True`, so `Example.RO` and `example.ro` were two distinct rows and an exact-match lookup for one silently missed the other. The service call sites already lowercased on the way in, but nothing enforced it, so any other writer (admin, data import, direct ORM) could store a mixed-case row that then failed to link — including the renew-item lookup `filter(name=domain_name.lower())`, where a stored `Example.RO` produced an unlinked item and a silently skipped renewal, the same failure class #430 fixed, reached from the data side. `save()` and the bulk ORM paths (`update`/`bulk_create`/`bulk_update` via a custom queryset) now strip and lowercase on every write, making the invariant structural, and a data migration canonicalizes existing rows — refusing to run, with the offending names, whenever several stored names map to the same canonical target (an existing lowercase twin or multiple mixed-case variants), rather than resolving a genuine duplicate by dropping a row (#442)
- **The API token expiry purge is indexed** — `APIToken` carried only a `(user, created_at)` index, so the daily purge (`filter(expires_at__lte=now)`) scanned the whole table; a new `(expires_at)` index turns it into a range scan, with plan-assertion tests run on the exact production statements. A `(user, expires_at)` composite for the per-user token-cap check was considered and dropped: cross-engine EXPLAIN experiments (PG16 at 20k and 200k rows) showed at most a marginal, contingent benefit (an index-only COUNT at scale) against per-user row counts kept small by the cap plus the purge itself — kept simple, re-add concurrently if production EXPLAIN proves need (#246)
- **Per-account backup listing returns the account's backups** — `list_backups(account=…)` narrowed the S3 *prefix* to `virtualmin-backups/{account.domain}/`, but archives and metadata are stored under `virtualmin-backups/{backup_id}/`, so the prefix matched no key and an account-scoped list always came back empty. The dangerous reading is that an empty list is indistinguishable from "this account has no backups": an operator checking a customer's restore options was told there was nothing to restore. The account filter now applies to `metadata["domain"]`, which leaves the existing key layout intact so archives already in S3 stay listable, and metadata with no `domain` is never attributed to an account. `list_backups` had no test coverage at all, which is why an always-empty result went unnoticed (#431)
- **Virtualmin nodes stranded by the HTTPS/pin requirement now have a remediation path** — `#334`'s fail-closed guard rejects any node without HTTPS and without either CA verification or a pinned SHA-256 certificate, but two classes of existing row had no way to satisfy it: servers with no linked `NodeDeployment` (the pin backfill reads the served certificate over verified SSH *through* the deployment, so those rows were skipped) and plain-HTTP servers (no pin helps; HTTPS must be enabled first). `pin_virtualmin_certificates --dry-run` now lists both classes explicitly instead of reporting only a failure count after the fact, and a new `--server-id <uuid> --fingerprint <sha256>` accepts an operator-supplied digest read on the node itself — deliberately not a network read, which would be the trust-on-first-use the command exists to avoid. The digest is normalized (colons and case accepted) and rejected if malformed, and pinning is refused outright for a plain-HTTP row so a pin can never imply a record is remediated. Separately, `--server-id` was declared `type=int` against a `UUIDField` primary key, so argparse rejected every real id before the command body ran and the option could never be used at all; `DEPLOYMENT.md` now documents the whole procedure, replacing an undefined "resolve those records" instruction (#337)
- **Node validation reports whether the panel certificate is actually CA-trusted** — the SSL check connected with `verify_mode=CERT_NONE` and `check_hostname=False`, then passed on `ssock.version()` alone, so *any* TLS listener reported "SSL/TLS enabled": self-signed, wrong-SAN and expired certificates were all indistinguishable from a valid one, and a node could reach `active` while serving a self-signed panel certificate with nothing recording it. The check now performs a real trust-chain handshake (system trust store, `CERT_REQUIRED`, SNI and hostname/SAN validation against the deployment fqdn) and reports the verdict, with the specific verification failure, in the result message and `details`. It deliberately still passes on an untrusted certificate: per #436 that gate is a rollout decision requiring a staging drill against a real node, since every node currently serving a self-signed certificate would begin failing validation the moment it lands. Covered by tests driving real handshakes against generated self-signed, CA-trusted, wrong-hostname and expired certificates rather than a mocked probe (#436)
- **Concurrent coupon removal no longer fails the request** — `CouponRedemption.mark_reversed()` checked `self.status == "applied"` in memory and then wrote blindly, so two callers holding separate instances of the same row (two concurrent `remove_coupon()` calls, or a retried request) both passed the guard and both ran the counter decrements. That drives `coupon.total_uses` from 1 to -1, violating its `total_uses >= 0` CHECK constraint and raising `IntegrityError` — so the second removal does not merely drift the bookkeeping, it aborts the whole transaction. Reachability, stated precisely: `remove_coupon` already locks the Order row before reading redemptions, so on PostgreSQL two concurrent removes serialize there and the loser reads an empty queryset; the race is reachable on SQLite dev (where `select_for_update` is a silent no-op) and for any future caller that does not hold the order lock. The status check is now a compare-and-swap in the database and the decrements are gated on its rowcount, so exactly one caller wins and the other reports that it did nothing; `remove_coupon` locks the coupon rows first (mirroring `apply_coupon`, in sorted id order to avoid deadlocking a concurrent multi-coupon removal) and only subtracts the order discount when the reversal actually happened, so a lost race can no longer discount the order twice. `mark_reversed()` now returns whether it performed the reversal, and refreshes a stale instance that lost the race. Reversal is also audited again: the compare-and-swap uses `QuerySet.update()`, which fires no signals, so the `coupon_redemption_reversed` event had stopped being written while applications kept being recorded. Two adjacent gaps found in review are tracked separately — reversal decrements whichever campaign the coupon points at now rather than the one charged (#481), and order cancellation never reverses redemptions at all (#482)
- **Async email attachments no longer bloat the task broker** — `EmailService.send_email(async_send=True)` forwarded full attachment bytes into `send_email_task` via the django-q2 Postgres ORM broker, where they were pickled + base64-encoded (~1.33×) into `django_q_ormq` and re-persisted on every retry, so a burst of PDF-bearing proforma emails could hold many multi-hundred-KB rows in the queue table at once; attachments whose total size exceeds the new `notifications.max_async_attachment_kb` setting (default 5 MB, `0` to disable) are now sent synchronously in-process so their bytes never transit the broker, with delivery preserved (#358).
- **`make install` works in a git worktree** — `scripts/patch_precommit_hook.py` hard-coded `HOOKS_DIR = pathlib.Path(".git/hooks")`, but in a worktree `.git` is a *file* pointing at the real gitdir, so the path didn't resolve and the hook-patch step failed the whole target with "run this script from the repo root" even though `uv sync` + pre-commit install had succeeded; the hooks directory is now resolved via `git rev-parse --git-path hooks`, which returns the shared hooks dir in both a normal checkout and a worktree (#386).
- **`make pre-commit` no longer prunes the virtualenv** — the fallback branch that bootstraps a missing pre-commit binary ran `uv sync --group dev`, but `uv sync` is declarative and removes every package outside the requested group set, so it stripped the `platform`, `prod`, and `portal` groups (65 packages, leaving exactly 124). `django-cors-headers` lives in the `platform` group and `corsheaders` is listed in platform's `INSTALLED_APPS`, so a subsequent `make dev` died inside `django.setup()` with `ModuleNotFoundError: No module named 'corsheaders'` before running a single migration, and because `dev-all` runs `make -j2` the result was a half-up stack with portal serving :8701 and platform dead on :8700; the branch now syncs `--all-groups`, which still provides the pre-commit binary it exists to install. Two setup docs that recommended equally narrow sync commands now point at `make install`.
- **Tailwind design system reconnected to the build** — Tailwind v4's CLI has no `--config` flag, so `shared/tailwind.preset.js` and both `tailwind.config.js` files were never read: building with a nonexistent config path still exited 0, and the two service bundles were byte-identical despite declaring different content globs and different `primary` hues. Three features silently resolved to nothing — every `primary-*` utility (20 distinct selectors across 50+ template references), the `@tailwindcss/forms` classes `form-input`/`form-select`/`form-checkbox` (23 references), and the `prose*` typography classes (8 templates, including ticket comment and reply bodies where server-rendered HTML has no per-element classes to fall back on). Neither plugin was even installed, so the preset would have thrown on `require()` had anything loaded it. Theming moves to CSS-first entry points at `assets/css/{platform,portal}.css` using `@theme` and `@plugin`, and the four inert config files are deleted — deleting them produced byte-identical CSS, which is what proves they contributed nothing. Only referenced-but-unemitted tokens were ported: the old configs also overrode Tailwind defaults (`borderRadius`, `fontSize` line-heights, `boxShadow`, `screens`) that never reached a browser, so the UI was built and reviewed against Tailwind's defaults and porting them now would restyle the app rather than fix it. Dark mode deliberately stays on `prefers-color-scheme` — the config asked for `darkMode: 'class'` but no `.dark` class is ever set on `<html>`, so switching strategies would stop all 578 `dark:` utilities from applying to anyone (ADR-0035 amended).
- **`make dev` fails fast on a pruned virtualenv** — when the `platform` dependency group is missing, Django died inside `django.setup()` with an opaque `ModuleNotFoundError` before running a single command, and `dev-all`'s `make -j2` turned that into a half-up stack (portal serving :8701, platform dead on :8700) that reads as a hang rather than a crash. A `check-venv-platform` guard now gates both `dev-platform` and `dev-all` and reports "run `make install`" in one line before anything starts, at a cost of 36ms. Portal deliberately gets no equivalent guard: its group holds only gunicorn, which `runserver` never uses, plus duplicates of base `[project].dependencies`, so it starts fine on a pruned venv and gating it would be a false alarm.
- **Coupon redemption records the post-discount order total** — `CouponRedemption.order_total_cents` documents the total *after* the discount, but `mark_applied` omitted the field from its `update_fields`, so the pre-discount total set at row creation was never overwritten and analytics/audit snapshots stored the wrong figure; the field is now persisted (#233). (The lock-scope race in the same issue is already fixed on master; the "re-apply a removed coupon" behavior is deferred to a separate design ticket because the `unique_coupon_per_order` DB constraint makes it a schema decision, not a bug fix.)
- **Virtualmin recovery/restore no longer reports fake success** — `test_recovery_capability` hardcoded `recovery_ready: True` and a "✅ ready for disaster recovery" message whenever a dry-run rebuild succeeded — even when its own connection test had failed — so operators trusted a 0%-functional capability; readiness now requires a reachable server and the message is explicit that a dry run only checks data presence. `restore_domain` collected per-component restore errors and then finalized as "Restore completed successfully" regardless, so a partial or total restore failure looked successful; component failures now roll back and surface as an error. First fail-closed pass on #326 — the remaining backup-path/S3-layout/rebuild findings are tracked as follow-ups.
- **Virtualmin backup pipeline is honest about the remote archive** — `_execute_full_backup` writes the archive on the remote Virtualmin host (the dest is passed to the `backup-domain` API), but the returned dest was discarded so `metadata['backup_path']` was never set, and verify/upload did a local `os.path.exists` on a reconstructed token-less path — every backup failed with a misleading "Backup file not found" and nothing reached S3. The real dest is now recorded and flagged `backup_location: remote`, and verify/upload fail with an explicit "remote retrieval not implemented (#326)" error instead of a false local failure; S3 metadata is written *after* the archive so an interrupted backup no longer leaves listable-but-unrestorable metadata. Third #326 pass (containment/honesty); the remote→local (or direct-to-S3) transfer and remote temp-archive cleanup are deferred to a dedicated transport ticket.
- **Virtualmin server rebuild can actually rebuild** — `rebuild_server_from_praho` called `create_virtualmin_account` for each surviving `VirtualminAccount` row, but that method rejects any domain that already has a row (`already exists in PRAHO`) — the row being rebuilt is the collision — so 100% of accounts failed and disaster recovery restored nothing exactly when it was needed. Rebuild now reuses the existing rows via a new `reprovision_virtualmin_account` path (fresh password, retargeted server, same server-side create workflow), and `current_domains` is reconciled from the authoritative active-account count instead of being clobbered to a stale value. Second pass on #326 (the CRITICAL rebuild finding).
...
- **Final `is_staff → is_staff_user` sweep sites resolved (#271 complete)** — the two sensitive boundaries are now settled: the HMAC/session-auth bypass for the ticket form's `/api/customers/` GET admits role-only staff (`staff_role` set, `is_staff=False`) via the canonical `is_staff_user`, fixing a broken customer-services dropdown; and credential-vault decryption is tightened to superusers and admin-role staff only, so support/billing/manager roles (and a bare `is_staff` flag) can no longer read Virtualmin/SSH/cloud secrets. Native `is_staff` now carries no authorization role in the platform (Django admin is disabled), matching the documented intent that business access derive from `staff_role` (#271).
- **Consistent staff classification for audit search suggestions** — `AuditSearchService.get_search_suggestions` gated user-search auto-complete on native `is_staff`, so role-only staff (`staff_role` set, `is_staff=False`) — who already pass the endpoint's `staff_required_strict` guard — were silently denied suggestions; the gate now uses the canonical `is_staff_user`, resolving one of the remaining `is_staff → is_staff_user` sweep sites (#271). (The two sensitive-boundary sites are resolved in the entry above.)
- **API abuse-protection proof** — order, catalog, and session-validation endpoint limits now execute on DRF function views instead of silently no-oping, protected HMAC views retain their global per-portal sustained and burst limits, and PostgreSQL CI proves concurrent API-token issuance cannot exceed the per-user cap while usage tracking writes at most once per interval (#229, #248)
...
- **Portal-keyed throttling on the last IP-keyed HMAC endpoint** — `update_customer_billing_address` carried the same defect #186 fixed for account creation: a per-view `@throttle_classes` *replaces* the global defaults, so `BurstAPIThrottle` both dropped the per-portal HMAC limits and — extending `UserRateThrottle` on an endpoint running `authentication_classes([])`, where `request.user` is `AnonymousUser` — degraded to client-IP keying that a caller could bypass by rotating source IPs. It now layers the portal-keyed throttles. A new guardrail walks every registered API view rather than a hand-maintained list, so the next HMAC endpoint with an IP-keyed throttle fails CI on its own; endpoints marked `@public_api_endpoint` are correctly exempt, since IP keying is the right choice where no portal identity exists. Shorthand rate-window parsing moved onto the shared throttle base, closing a startup/request-time asymmetry in which an env value like `THROTTLE_RATE_PORTAL_HMAC=200/10s` passed deploy-time validation and then raised `KeyError` on the first live request — `PortalHMACRateThrottle` and `CustomerRateThrottle`, both global defaults, were exposed. Per-portal credentials and a portal registry, which would make `X-Portal-Id` unforgeable by a secret holder, remain tracked in #277 (#186, #277)
...
- **Insecure `get_client_ip` in price sealing** (#57) — Deleted hand-rolled `get_client_ip()` and `_is_valid_ip()` from `price_sealing.py` that trusted proxy headers from any source and used leftmost-IP (`split(",")[0]`); replaced with `get_safe_client_ip()` (trusted-proxy-aware, rightmost-trusted-hop) in views and serializers; added 6 regression tests and 3 new security scanner patterns for hand-rolled IP extraction
- **CNP validation tautological check** (#64) — Extracted `CNPValidator` from `billing/efactura/b2c.py` to `common/cnp_validator.py` for reuse across platform and portal; fixed portal tautological digit check (`"1234567890"` → `"123456789"`, digit 0 is not a valid century marker); upgraded platform form to full semantic validation (checksum, birth date, county codes); 21 tests added
