...
- **API token expiry queries are indexed** — `APIToken` carried only a `(user, created_at)` index, so neither expiry access path was served: the daily purge (`filter(expires_at__lte=now)`) scanned the whole table, and the per-user live-token count that enforces the token cap could use only the `user` prefix before filtering expiry in memory — a cost paid on every `obtain_token` call. Adds `(expires_at)` and `(user, expires_at)`; both additive and reversible, with the existing index left in place for the token-listing view (#246)
