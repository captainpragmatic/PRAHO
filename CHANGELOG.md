...
- **Concurrent coupon removal no longer fails the request** — `CouponRedemption.mark_reversed()` checked `self.status == "applied"` in memory and then wrote blindly, so two callers holding separate instances of the same row (two concurrent `remove_coupon()` calls, or a retried request) both passed the guard and both ran the counter decrements. That drove `coupon.total_uses` from 1 to -1, violating its `total_uses >= 0` CHECK constraint and raising `IntegrityError` — so the second removal did not merely drift the bookkeeping, it aborted the whole transaction and surfaced as a server error. The status check is now a compare-and-swap in the database and the decrements are gated on its rowcount, so exactly one caller wins and the other reports that it did nothing; `remove_coupon` locks the coupon rows first (mirroring `apply_coupon`, in sorted id order to avoid deadlocking a concurrent multi-coupon removal) and only subtracts the order discount when the reversal actually happened, so a lost race can no longer discount the order twice. `mark_reversed()` now returns whether it performed the reversal, and refreshes a stale instance that lost the race. The append-only redemption ledger that would allow re-applying a removed coupon remains open in #421 (#421)
