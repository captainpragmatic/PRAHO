...
- **Domain renewal page markup and summary calculator** — `domain_renew.html` was stored with its lower half collapsed onto one line of literal `\n`/`\"` escape sequences, rendering broken markup and a dead renewal-summary script; the file is restored to real markup and the missing `add_years` template filter (used for the projected new-expiry date, with leap-day safety) is implemented so the "New expiry" line no longer raises `Invalid filter` (#285)
