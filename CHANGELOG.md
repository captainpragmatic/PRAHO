...
- **e-Factura duplicate-upload prevention** — submission decisions now use committed, leased row-lock claims before any ANAF POST; concurrent callers become idempotent no-ops, claimed XML bytes remain immutable, upload timeouts/5xx responses and expired workers are quarantined for reconciliation instead of replayed, and the compliance dashboard exposes that operator action queue (#351)
