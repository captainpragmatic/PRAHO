...
- **Domain TLD and renewal-page correctness** — configured multi-label suffixes now win by longest match across registration, transfer, availability, order, model-validation, and sample-data paths; registration periods enforce each TLD's own limits before registrar or order writes; and the renewal page renders valid markup with exact calendar-year expiry previews (#237, #285)
