...
- **The API token expiry purge is indexed** — `APIToken` carried only a `(user, created_at)` index, so the daily purge (`filter(expires_at__lte=now)`) scanned the whole table; a new `(expires_at)` index turns it into a range scan, with a plan-assertion test. The other #246 candidate — a `(user, expires_at)` composite for the per-user token-cap check — was deliberately dropped: EXPLAIN shows the production null-OR-future shape never uses it (the planner takes the plain user FK index, and per-user rows are bounded by the cap itself), so it would have been pure write amplification; a test pins its absence alongside the real-shape plan (#246)
