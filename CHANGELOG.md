...
- **Audited billing operator controls** — billing/admin staff can edit the live payment retry cadence and dunning-email behavior with validation, optimistic locking, required reasons, and rollback-safe audit evidence; the same Settings surface exposes read-only invoice sequence state and a guarded legal-series rotation with no free-form counter edits or prefix reuse (#378, #379)
...
- **Recurring-charge revocation ordering** — final authorization and Stripe submission now share a PostgreSQL boundary with mandate withdrawal, cancellation, renewal opt-out, and the global kill switch; revocation-first attempts abort before Stripe, while already-submitted attempts finish before the opt-out returns. The kill-switch row is read under a lock that releases before the gateway round-trip, so one hung Stripe call can neither serialize other customers' charges nor stall the kill switch; a reservation resumed from a prior attempt is never failed inline, since its charge may have reached Stripe already — its outcome is left to webhook reconciliation (#316)
