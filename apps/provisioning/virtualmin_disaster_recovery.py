...
class VirtualminDisasterRecoveryService:
                ...
                try:
                    ...
                    if result.is_ok():
                        ...
                        if account.status == "suspended":
                            provisioning_service.suspend_account(
                                new_account, 
                                "Restored as suspended from PRAHO data"
                            )
    ...
    def verify_praho_data_integrity(self) -> Result[dict[str, Any], str]:
        ...
        try:
            ...
            accounts_without_customers = VirtualminAccount.objects.filter(
                service__customer__isnull=True
            )
            ...
            misconfigured_servers = VirtualminServer.objects.filter(
                api_username="",
            ...
            integrity_status = "excellent" if recovery_percentage >= EXCELLENT_RECOVERY_THRESHOLD else \
                             "good" if recovery_percentage >= GOOD_RECOVERY_THRESHOLD else \
                             "warning" if recovery_percentage >= WARNING_RECOVERY_THRESHOLD else "critical"
        ...
        for issue in issues:
            if issue["issue"] == "accounts_without_services":
                recommendations.append(
                    "🔗 Link orphaned accounts to PRAHO services or mark for cleanup"
                )
            elif issue["issue"] == "accounts_without_customers":
                recommendations.append(
                    "👤 Ensure all services have valid customer associations"
                )
            elif issue["issue"] == "servers_without_credentials":
                recommendations.append(
                    "🔑 Configure API credentials for all Virtualmin servers"
                )
