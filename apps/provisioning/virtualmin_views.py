...
def virtualmin_server_detail(request: HttpRequest, server_id: str) -> HttpResponse:
    ...
    recent_accounts = VirtualminAccount.objects.filter(
        server=server
    ).select_related('service', 'service__customer').order_by('-created_at')[:10]
    
    # Get recent provisioning jobs
    recent_jobs = VirtualminProvisioningJob.objects.filter(
        server=server
    ...
    if server.status == "active":
        try:
            ...
            if domains_result.is_ok():
                ...
                logger.info(f"✅ [ServerDetail] Listed {len(actual_domains)} domains from {server.hostname}, "
...
def virtualmin_accounts_list(request: HttpRequest) -> HttpResponse:
    ...
    # Get base queryset
    accounts = VirtualminAccount.objects.select_related(
        'server', 'service', 'service__customer'
    ).order_by('-created_at')
    ...
    if server_filter:
        accounts = accounts.filter(server_id=server_filter)
    if search_query:
        accounts = accounts.filter(
            models.Q(domain__icontains=search_query) |
            models.Q(service__customer__name__icontains=search_query)
        )
...
def virtualmin_account_detail(request: HttpRequest, account_id: str) -> HttpResponse:
    ...
    account = get_object_or_404(
        VirtualminAccount.objects.select_related('server', 'service', 'service__customer'),
        id=account_id
    )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            backup_result = backup_management.create_backup_job(
                ...
                initiated_by=f"staff:{request.user.email}"
            )
            ...
            if backup_result.is_ok():
                ...
                messages.success(
                    request,
                    f"Backup job created successfully! Job ID: {job.id}"
                )
                return redirect('provisioning:virtualmin_backup_status', job_id=job.id)
            else:
                messages.error(
                    request,
                    f"Failed to create backup: {backup_result.unwrap_err()}"
                )
    else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if restore_result.is_ok():
                ...
                messages.success(
                    request,
                    f"Restore job created successfully! Job ID: {job.id}"
                )
                return redirect('provisioning:virtualmin_backup_status', job_id=job.id)
            else:
                messages.error(
                    request,
                    f"Failed to create restore: {restore_result.unwrap_err()}"
                )
    else:
...
def virtualmin_backup_status(request: HttpRequest, job_id: str) -> HttpResponse:
    ...
    if request.headers.get('HX-Request'):
        return render(request, 'provisioning/virtualmin/partials/job_status.html', {
            'job': job,
    ...
    context = {
        ...
        'account_url': reverse('provisioning:virtualmin_account_detail', args=[job.account.id]),
        'is_complete': job.status in ['completed', 'failed']
    }
...
def virtualmin_server_create(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Virtualmin server '{server.name}' created successfully!"
            )
...
def virtualmin_server_edit(request: HttpRequest, server_id: str) -> HttpResponse:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Server '{server.name}' updated successfully!"
            )
            return redirect('provisioning:virtualmin_server_detail', server_id=server.id)
    else:
...
def virtualmin_server_test_connection(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        result = management_service.test_server_connection(temp_server)
...
def virtualmin_server_health_check(request: HttpRequest, server_id: str) -> HttpResponse:
    ...
    try:
        ...
        if health_result.is_ok():
            ...
            messages.success(
                request,
                f"Health check completed. Server status: {health_data.get('status', 'Unknown')}"
            )
            ...
            messages.error(
                request,
                f"Health check failed: {health_result.unwrap_err()}"
            )
    ...
    if request.headers.get('HX-Request'):
        ...
        return render(request, 'provisioning/virtualmin/partials/health_status_card.html', {
            'server': server,
            'health_status': health_status
        })
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # Execute bulk action
                if action == 'backup':
                    success_count = _execute_bulk_backup(accounts, form.cleaned_data)
                    messages.success(
                        request, 
                        f"Backup jobs created for {success_count}/{len(accounts)} accounts"
                    )
                elif action == 'suspend':
                    success_count = _execute_bulk_suspend(accounts)
                    messages.success(
                        request,
                        f"Suspended {success_count}/{len(accounts)} accounts"
                    )
                elif action == 'activate':
                    success_count = _execute_bulk_activate(accounts)
                    messages.success(
                        request,
                        f"Activated {success_count}/{len(accounts)} accounts"
                    )
                elif action == 'health_check':
                    success_count = _execute_bulk_health_check(accounts)
                    messages.success(
                        request,
                        f"Health checks completed for {success_count}/{len(accounts)} accounts"
                    )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request,
                    f"Account {account.domain} created successfully!"
                )
                return redirect('provisioning:virtualmin_account_detail', account_id=account.id)
            except Exception as e:
                messages.error(
