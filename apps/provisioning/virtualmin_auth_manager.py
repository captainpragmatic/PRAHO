...
class VirtualminAuthenticationManager:
    ...
    def execute_virtualmin_command(
        ...
        force_method: AuthMethod | None = None
        ...
        for method in methods_to_try:
            ...
            try:
                ...
                if result.is_ok():
                    ...
                    logger.info(
                        f"✅ [Virtualmin Auth] {method.value} succeeded for {program} "
        ...
        with get_virtualmin_auth_manager(server) as auth_manager:
            ...
            if acl_result and acl_result.success:
                ...
                fallback_working = any(
                    result.success for method, result in health_results.items()
                    if method != AuthMethod.ACL.value
                )
