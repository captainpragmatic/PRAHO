...
def get_virtualmin_config() -> dict:
    ...
    return {
        ...
        'auth_health_check_interval': SettingsService.get_setting('virtualmin.auth_health_check_interval_seconds', 3600),
...
class VirtualminResponseParser:
    ...
    def _parse_text_response(response_text: str, program: str) -> dict[str, Any]:
        ...
        success_patterns = [
            "successfully", "completed", "created", "deleted", "modified"
        ]
        ...
        if program.startswith("list-"):
            ...
            return {
                "success": True,
                "data": {"items": lines, "raw_response": response_text},
                "error": ""
            }
...
class VirtualminGateway:
    ...
    def _get_credentials(self, reason: str = "Virtualmin API call") -> Result[tuple[str, str], str]:
        ...
        if vault:
            vault_result = vault.get_credential(
                ...
                service_identifier=self.server.hostname,
                reason=reason
            )
            ...
            if vault_result.is_ok():
                ...
                logger.warning(f"⚠️ [Virtualmin Gateway] Vault failed for {self.server.hostname}: {vault_result.unwrap_err()}")
        ...
        try:
            ...
            if env_username and env_password:
                logger.info(f"🔐 [Virtualmin Gateway] Using environment credentials for {self.server.hostname} (migration needed)")
    ...
    def call_with_auth_fallback(
        ...
        if use_fallback_auth:
            ...
            return self._get_auth_manager().execute_virtualmin_command(
                program, 
                parameters or {}
            )
        else:
    ...
    def _validate_ssl_certificate(self, response: requests.Response) -> bool:
        ...
        try:
            ...
            if expected != actual:
                logger.error(
                    f"SSL certificate mismatch for {self.server.hostname}. "
    ...
    def call(
        ...
        if not self._check_rate_limit(program):
            return Err(VirtualminRateLimitedError(
                f"Rate limit exceeded for {program}",
        ...
        for attempt in range(VIRTUALMIN_MAX_RETRIES):
            try:
                ...
                # Parse response
                parsed_data = VirtualminResponseParser.parse_response(
                    response.text, 
                    program
                )
                ...
                logger.info(
                    f"✅ [Virtualmin] {program} completed in {execution_time:.2f}s "
                ...
                last_error = e
                logger.warning(
                    f"⚠️ [Virtualmin] Attempt {attempt + 1}/{VIRTUALMIN_MAX_RETRIES} failed: {e}"
                )
        ...
        logger.error(
            f"❌ [Virtualmin] {program} failed after {execution_time:.2f}s: {error_msg}"
        )
    ...
    def _make_request(self, params: dict[str, Any], attempt: int) -> requests.Response:
        ...
        try:
            ...
            raise VirtualminTransientError(
                f"Read timeout from {self.server.hostname}",
                self.server.hostname
            ) from e
        except requests.exceptions.ConnectionError as e:
            raise VirtualminTransientError(
                f"Connection error to {self.server.hostname}: {e}",
                self.server.hostname
            ) from e
