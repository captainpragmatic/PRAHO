...
from .virtualmin_gateway import VirtualminGateway
...
class VirtualminBackupService:
    ...
    def backup_domain(
        ...
        try:
            ...
            backup_metadata = self._initialize_backup_metadata(
                account, config.backup_type, backup_id, config
            )
            ...
            if config.backup_type == "full":
                backup_result = self._execute_full_backup(
                    account, backup_id, backup_metadata, config
                )
            elif config.backup_type == "incremental":
                backup_result = self._execute_incremental_backup(
                    account, backup_id, backup_metadata, config
                )
            elif config.backup_type == "config_only":
                backup_result = self._execute_config_backup(
                    account, backup_id, backup_metadata
                )
            else:
                return Err(f"Unsupported backup type: {config.backup_type}")
                ...
                for obj in page.get('Contents', []):
                    if obj['Key'].endswith('.json'):  # Metadata files
                        try:
                            ...
                            backup_date = timezone.datetime.fromisoformat(metadata['created_at'])
    ...
    def _get_s3_client(self) -> boto3.client:
        ...
        if self._s3_client is None:
            aws_access_key = self.settings.get('backup.aws_access_key_id')
            aws_secret_key = self.settings.get('backup.aws_secret_access_key')
            aws_region = self.settings.get('backup.aws_region', 'eu-west-1')
    ...
    def _get_backup_bucket(self) -> str:
        ...
        if self._backup_bucket is None:
            self._backup_bucket = self.settings.get('backup.s3_bucket_name')
    ...
    def _initialize_backup_metadata(
        self, account: VirtualminAccount, backup_type: str, backup_id: str,
        config: BackupConfig
    ) -> dict[str, Any]:
    ...
    def _validate_backup_preconditions(self, account: VirtualminAccount) -> Result[None, str]:
        ...
        gateway = VirtualminGateway(self.server)
    ...
    def _execute_full_backup(
        ...
        try:
            ...
            if not config.include_databases:
                backup_params['skip-features'] = backup_params.get('skip-features', '') + ',mysql'
            if not config.include_files:
                backup_params['skip-features'] = backup_params.get('skip-features', '') + ',dir'
            if not config.include_ssl:
                backup_params['skip-features'] = backup_params.get('skip-features', '') + ',ssl'
    ...
    def _execute_incremental_backup(
        ...
        if last_backup_result.is_err():
            ...
            return self._execute_full_backup(
                account, backup_id, metadata, config
            )
    ...
    def _restore_email_data(self, gateway: VirtualminGateway, account: VirtualminAccount, backup_path: str) -> Result[None, str]:
        """Restore email stores and configuration."""
        return Ok(None)
        
    def _restore_database_data(self, gateway: VirtualminGateway, account: VirtualminAccount, backup_path: str) -> Result[None, str]:
        """Restore database data and configuration."""
        return Ok(None)
        
    def _restore_file_data(self, gateway: VirtualminGateway, account: VirtualminAccount, backup_path: str) -> Result[None, str]:
        """Restore web files and uploads."""
        return Ok(None)
        
    def _restore_ssl_certificates(self, gateway: VirtualminGateway, account: VirtualminAccount, backup_path: str) -> Result[None, str]:
        """Restore SSL certificates and private keys."""
        return Ok(None)
        
    def _verify_restore_integrity(self, gateway: VirtualminGateway, account: VirtualminAccount, metadata: dict[str, Any]) -> Result[None, str]:
    ...
    def _finalize_restore_summary(self, account: VirtualminAccount, backup_id: str, restore_id: str, metadata: dict[str, Any]) -> dict[str, Any]:
