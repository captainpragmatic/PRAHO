...
def handle_service_plan_created_or_updated(sender: type[ServicePlan], instance: ServicePlan, created: bool, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if created:
            ...
            if old_monthly_price and abs(float(instance.price_monthly) - old_monthly_price) > PRICE_CHANGE_DETECTION_THRESHOLD:
...
def handle_server_created_or_updated(sender: type[Server], instance: Server, created: bool, **kwargs: Any) -> None:
    ...
    try:
        ...
        if instance.max_services and instance.active_services_count >= instance.max_services * MAX_SERVICES_WARNING_THRESHOLD:
...
def handle_service_created_or_updated(sender: type[Service], instance: Service, created: bool, **kwargs: Any) -> None:
    ...
    try:
        ...
        if created:
            ...
            logger.info(f"⚙️ [Provisioning] Created service: {instance.service_name} for {instance.customer.get_display_name()}")
...
def handle_service_relationship_created_or_updated(sender: type[ServiceRelationship], instance: ServiceRelationship, created: bool, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if created:
            ...
            logger.info(f"🔗 [Provisioning] Service relationship created: {instance.parent_service.service_name} → {instance.child_service.service_name}")
            
            # Auto-provision child service if configured
            if instance.auto_provision and instance.child_service.status == 'pending':
                _trigger_auto_provisioning(instance.child_service, f"Auto-provision via relationship with {instance.parent_service.service_name}")
...
def handle_service_group_created_or_updated(sender: type[ServiceGroup], instance: ServiceGroup, created: bool, **kwargs: Any) -> None:
    """
...
def handle_provisioning_task_created_or_updated(sender: type[ProvisioningTask], instance: ProvisioningTask, created: bool, **kwargs: Any) -> None:
    """
...
def store_original_provisioning_task_values(sender: type[ProvisioningTask], instance: ProvisioningTask, **kwargs: Any) -> None:
...
def handle_service_domain_created_or_updated(sender: type[ServiceDomain], instance: ServiceDomain, created: bool, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        if created:
            ...
            logger.info(f"🌐 [Provisioning] Domain bound: {instance.full_domain_name} to {instance.service.service_name}")
...
def _handle_service_plan_pricing_change(plan: ServicePlan, old_price: float, new_price: float) -> None:
    ...
    try:
        ...
        logger.info(f"💰 [Provisioning] Plan pricing changed: {plan.name} {old_price:.2f} → {new_price:.2f} RON ({change_percentage:.1f}% change)")
...
def _handle_server_overload_alert(server: Server, resource_type: str, usage_percentage: float) -> None:
    ...
    try:
        logger.warning(f"⚠️ [Provisioning] Server {resource_type.upper()} overload: {server.name} ({usage_percentage:.1f}%)")
...
def _handle_long_provisioning_task(task: ProvisioningTask, duration_seconds: float) -> None:
    ...
    logger.warning(f"⏱️ [Provisioning] Long-running task: {task.get_task_type_display()} ({duration_seconds/60:.1f} minutes)")
