...
class RomanianDocumentPDFGenerator:
    ...
    def _render_company_information(self) -> None:
        ...
        self.canvas.drawString(
            2*cm, y_pos - 1*cm, 
            f"{company_info['address']}, {company_info['city']}, {company_info['country']}"
        )
        self.canvas.drawString(
            2*cm, y_pos - 1.5*cm, 
            str(_t("Tax ID: {cui}")).format(cui=company_info['cui'])
        )
        self.canvas.drawString(
            2*cm, y_pos - 2*cm, 
            str(_t("Email: {email}")).format(email=company_info['email'])
        )
    ...
    def _render_client_information(self) -> None:
        ...
        if self.document.bill_to_tax_id:
            self.canvas.drawString(
                11*cm, y_pos - 1.5*cm, 
                str(_t("Tax ID: {tax_id}")).format(tax_id=self.document.bill_to_tax_id)
            )
            
        if self.document.bill_to_email:
            self.canvas.drawString(
                11*cm, y_pos - 2*cm, 
                str(_t("Email: {email}")).format(email=self.document.bill_to_email)
            )
    ...
    def _render_totals_section(self) -> None:
        ...
        self.canvas.drawString(
            12*cm, totals_y, 
            str(_t("Subtotal: {amount} RON")).format(amount=f"{self.document.subtotal:.2f}")
        )
        self.canvas.drawString(
            12*cm, totals_y - 0.5*cm, 
        ...
        self.canvas.drawString(
            12*cm, totals_y - 1*cm, 
            str(total_label).format(amount=f"{self.document.total:.2f}")
        )
    ...
    def _render_document_footer(self) -> None:
        ...
        self.canvas.drawString(
            2*cm, 1.5*cm, 
            str(_t("Generated automatically by {platform}")).format(platform=company_info['name'])
        )
...
class RomanianInvoicePDFGenerator(RomanianDocumentPDFGenerator):
    ...
    def _render_document_details(self) -> None:
        ...
        self.canvas.drawString(
            2*cm, self.height - 5*cm, 
            str(_t("Number: {number}")).format(number=self.invoice.number)
        )
        
        if self.invoice.issued_at:
            self.canvas.drawString(
                2*cm, self.height - 5.5*cm, 
                str(_t("Issue date: {date}")).format(date=self.invoice.issued_at.strftime('%d.%m.%Y'))
            )
            
        if self.invoice.due_at:
            self.canvas.drawString(
                2*cm, self.height - 6*cm, 
        ...
        self.canvas.setFont("Helvetica-Bold", 10)
        self.canvas.drawString(
            14*cm, self.height - 5*cm, 
            str(_t("Status: {status}")).format(status=self.invoice.status.upper())
        )
    ...
    def _render_status_information(self, totals_y: float) -> None:
        ...
        if self.invoice.status != 'paid':
            ...
            self.canvas.drawString(
                2*cm, totals_y - 2*cm, 
                str(_t("⚠️  Unpaid invoice - Due: {date}")).format(date=due_date_str)
            )
...
class RomanianProformaPDFGenerator(RomanianDocumentPDFGenerator):
    ...
    def _render_document_details(self) -> None:
        ...
        self.canvas.drawString(
            2*cm, self.height - 5*cm, 
            str(_t("Number: {number}")).format(number=self.proforma.number)
        )
        self.canvas.drawString(
            2*cm, self.height - 5.5*cm, 
