...
class RefundService:
    ...
    def _validate_invoice_refund(invoice: Any, refund_data: RefundData) -> Result[None, str]:
        ...
        if refund_type in (RefundType.PARTIAL, 'partial'):
            ...
            eligibility_data = eligibility.unwrap()
            max_refundable = eligibility_data.get('max_refund_amount_cents', 
                                                getattr(invoice, 'total_cents', 11900))
            if amount > max_refundable:
    ...
    def _check_entity_refund_eligibility(entity: Any, entity_type: str) -> Result[RefundEligibility, str]:
        ...
        return Result.ok(RefundEligibility(
            is_eligible=True, 
            max_refund_amount_cents=max_refundable, 
    ...
    def _validate_order_refund_eligibility(order: Any, refund_data: RefundData) -> Result[dict, str]:
        ...
        try:
            ...
            if hasattr(order, 'status'):
                ...
                if not status_result['is_eligible'] or status_result['is_eligible']:
            ...
            return Result.ok(RefundService._create_eligibility_result(
                False, 'Order not eligible', 0, already_refunded
            ))
        except Exception as e:
            return Result.err(f"Failed to validate eligibility: {e!s}")
    
    @staticmethod
    def _check_order_status_eligibility(status: str, already_refunded: int, total_cents: int, max_refundable: int) -> dict[str, Any]:
    ...
    @staticmethod
    def _validate_order_partial_amount(refund_data: RefundData, max_refundable: int, already_refunded: int) -> dict[str, Any]:
        """Validate partial refund amount constraints"""
        refund_type = refund_data.get('refund_type', 'full')
        if refund_type not in (RefundType.PARTIAL, 'partial'):
            return RefundService._create_eligibility_result(
    ...
    @staticmethod
    def _create_eligibility_result(is_eligible: bool, reason: str, max_refund_cents: int, already_refunded_cents: int) -> dict[str, Any]:
    ...
    @staticmethod
    def _validate_invoice_refund_eligibility(invoice: Any, refund_data: RefundData) -> Result[dict, str]:
        ...
        try:
            ...
            # Check invoice status - draft invoices should not be eligible
            if hasattr(invoice, 'status'):
                if invoice.status == 'draft':
                    return Result.ok({
                        'is_eligible': False,
                        'reason': "Cannot refund invoice in 'draft' status",
                        'max_refund_amount_cents': 0,
                        'already_refunded_cents': already_refunded
                    })
                elif invoice.status in ['paid', 'completed']:
    ...
    def _process_bidirectional_refund(order: Any = None, invoice: Any = None, refund_id: Any = None, refund_data: RefundData = None, **kwargs: Any) -> Result[dict, str]:
        ...
        try:
            ...
            refund_result = RefundService._create_refund_record(RefundRecordParams(
                refund_id=refund_id,
                order=order,
                invoice=invoice,
    ...
    def _extract_refund_amount(refund_data: RefundData | None, kwargs: dict[str, Any]) -> int:
        ...
        return kwargs.get('refund_amount_cents', refund_data.get('amount_cents', 0) if refund_data else 0)
    ...
    def _create_refund_record(params: RefundRecordParams) -> Result[None, str]:
        ...
        try:
            ...
            refund = Refund.objects.create(
                ...
                reference_number=refund_data.get('reference', f"REF-{refund_id}") if refund_data else f"REF-{refund_id}",
                status='pending'
            )
    ...
    @staticmethod
    def _process_entity_updates(order: Any, invoice: Any, refund_id: Any, refund_data: RefundData | None) -> Result[dict, str]:
        ...
        if invoice:
            ...
            try:
                invoice_update_result = RefundService._update_invoice_refund_status(invoice, refund_data)
    ...
    @staticmethod
    def _process_payment_refund_if_exists(order: Any, invoice: Any, refund_data: RefundData | None) -> Result[dict, str] | None:
    ...
    @staticmethod
    def _update_order_refund_status(order: Any, refund_amount_cents: int | None = None, refund_data: RefundData | None = None) -> Result[None, str]:
        ...
        try:
            ...
            if hasattr(order, 'status'):
                ...
                current_amount = refund_amount_cents if refund_amount_cents is not None else (refund_data.get('amount_cents', 0) if refund_data else 0)
    ...
    @staticmethod
    def _update_invoice_refund_status(invoice: Any, refund_amount_cents: int | None = None, refund_data: RefundData | None = None) -> Result[None, str]:
    ...
    def _get_order_refunded_amount(order: Any) -> int:
        ...
        try:
            return Refund.objects.filter(order=order).aggregate(
    ...
    def _get_invoice_refunded_amount(invoice: Any) -> int:
        ...
        # Check related refunds
        try:
            return Refund.objects.filter(invoice=invoice).aggregate(
                total=Sum('amount_cents', default=0)
            )['total']
        except Exception:
            return 0
        ...
        try:
            ...
            if hasattr(order, 'status'):
                if order.status == 'draft':
                    return Result.ok(RefundEligibility(
                        is_eligible=False,
                        reason="Cannot refund order in 'draft' status",
                        max_refund_amount_cents=0
                    ))
                elif order.status not in ['paid', 'completed']:
                    return Result.ok(RefundEligibility(
                        is_eligible=False,
                        reason='Order not in refundable state',
                        max_refund_amount_cents=0
                    ))
            ...
            if refund_type in (RefundType.PARTIAL, 'partial'):
                ...
                if amount > max_refundable:
                    return Result.ok(RefundEligibility(
                        is_eligible=False,
                        reason='Refund amount exceeds maximum refundable amount',
                        max_refund_amount_cents=max_refundable
                    ))
        ...
        try:
            ...
            if hasattr(invoice, 'status'):
                if invoice.status == 'draft':
                    return Result.ok(RefundEligibility(
                        is_eligible=False,
                        reason='Invoice is in draft status and cannot be refunded',
                        max_refund_amount_cents=0
                    ))
                elif invoice.status not in ['paid', 'completed']:
                    return Result.ok(RefundEligibility(
                        is_eligible=False,
                        reason='Invoice not in refundable state',
                        max_refund_amount_cents=0
                    ))
    ...
    @staticmethod
    def _process_payment_refund(payment: Any = None, refund_data: RefundData = None, **kwargs: Any) -> Result[dict, str]:
        ...
        try:
            ...
            log_security_event(
                ...
                details={
                    ...
                    "refund_amount_cents": refund_amount_cents or (refund_data.get('amount_cents', 0) if refund_data else 0),
                    "refund_type": refund_data.get('refund_type', 'full') if refund_data else 'full',
                    "critical_financial_operation": True,
                }
            )
            ...
            return Result.ok({
                ...
                "total_refunded_cents": refund_amount_cents or (refund_data.get('amount_cents', 0) if refund_data else 0),
            ...
            refunds.extend([
                {
                    ...
                    'reason': str(refund.reason).lower().replace('refundreason.', '') if hasattr(refund.reason, 'value') else refund.reason,
