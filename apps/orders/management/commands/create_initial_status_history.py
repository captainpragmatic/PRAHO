...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        orders_without_history = Order.objects.filter(
            status_history__isnull=True
        ).distinct()

        if not orders_without_history.exists():
            self.stdout.write(
                self.style.SUCCESS('All orders already have status history records.')
            )
            return
        ...
        if options['dry_run']:
            self.stdout.write(
                self.style.WARNING(
                    f'DRY RUN: Would create initial status history for {count} orders:'
                )
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully created initial status history for {created_count} orders.'
            )
        )
