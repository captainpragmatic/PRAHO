...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: ArgumentParser) -> None:
        parser.add_argument("--service", type=str, help="Rotate credentials for specific service type")

        parser.add_argument("--identifier", type=str, help="Rotate credential for specific identifier")

        parser.add_argument(
            "--test-only", action="store_true", help="Test rotation without actually changing credentials"
        )

        parser.add_argument("--force", action="store_true", help="Force rotation even if not expired")

        parser.add_argument("--max-age-days", type=int, default=30, help="Maximum age in days before forcing rotation")

    def handle(self, *args: Any, **options: Any) -> None:
        """Execute credential rotation"""

        self.stdout.write(self.style.SUCCESS("🔄 Starting credential rotation..."))
        ...
        try:
            ...
            if options["service"] and options["identifier"]:
                ...
                self._rotate_specific_credential(
                    vault, options["service"], options["identifier"], options["test_only"], options["force"]
                )
            else:
                # Rotate expired/aging credentials
                self._rotate_aging_credentials(
                    vault, options["service"], options["max_age_days"], options["test_only"], options["force"]
                )

            # Show rotation summary
            self._show_rotation_summary(vault)

            self.stdout.write(self.style.SUCCESS("✅ Credential rotation complete!"))

        except Exception as e:
            raise CommandError(f"Rotation failed: {e}") from e

    def _rotate_specific_credential(
        self, vault: CredentialVault, service_type: str, identifier: str, test_only: bool, force: bool
    ) -> None:
        """Rotate a specific credential"""
        self.stdout.write(f"🎯 Rotating {service_type}:{identifier}...")

        if test_only:
            self.stdout.write(self.style.WARNING("⚠️ TEST MODE: No actual rotation will occur"))

            # Just test if credential exists and can be accessed
            result = vault.get_credential(
                service_type=service_type, service_identifier=identifier, reason="Rotation test"
            )

            if result.is_ok():
                self.stdout.write(self.style.SUCCESS("✅ Credential found and accessible"))
            else:
                self.stdout.write(self.style.ERROR(f"❌ Credential test failed: {result.unwrap_err()}"))
            return

        # Perform actual rotation
        rotation_data = RotationData(
            service_type=service_type, service_identifier=identifier, reason="Manual rotation via management command"
        )
        result = vault.rotate_credential(rotation_data)  # type: ignore[assignment]

        if result.is_ok():
            self.stdout.write(self.style.SUCCESS(f"✅ Successfully rotated {service_type}:{identifier}"))
        else:
            self.stdout.write(self.style.ERROR(f"❌ Rotation failed: {result.unwrap_err()}"))

    def _rotate_aging_credentials(
        self, vault: CredentialVault, service_filter: str | None, max_age_days: int, test_only: bool, force: bool
    ) -> None:
        ...
        if test_only:
            self.stdout.write(self.style.WARNING("⚠️ TEST MODE: Listing credentials that would be rotated"))
        ...
        for cred in credentials_to_rotate:
            ...
            if result.is_ok():
                ...
                failure_count += 1
                self.stdout.write(self.style.ERROR(f"  ❌ Failed: {result.unwrap_err()}"))

        self.stdout.write(self.style.SUCCESS(f"📊 Rotation Summary: {success_count} success, {failure_count} failed"))
    ...
    def _show_rotation_summary(self, vault: CredentialVault) -> None:
        ...
        if status["failed_rotations"] > 0:
            ...
            for cred in failed_creds:
                self.stdout.write(f"  • {cred} - {cred.rotation_failure_count} failures")
        ...
        if expiring:
            ...
            for cred in expiring:
                ...
                urgency_icon = (
                    "🚨" if days_left <= CRITICAL_EXPIRY_DAYS else "⚠️" if days_left <= WARNING_EXPIRY_DAYS else "🟡"
                )
