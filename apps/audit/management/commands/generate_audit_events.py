...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument("--count", type=int, default=30, help="Number of random events to generate (default: 30)")
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if not users:
            self.stdout.write(self.style.WARNING("⚠️  No users found. Creating a test user..."))
            test_user = User.objects.create_user(email="test@example.com", first_name="Test", last_name="User")
            users = [test_user]
        ...
        for i in range(count):
            try:
                ...
                self.stdout.write(self.style.ERROR(f"❌ Error creating event {i + 1}: {e}"))
                continue

        self.stdout.write(self.style.SUCCESS(f"🎉 Successfully generated {events_created} random audit events!"))

        # Show pagination info
        total_events = AuditEvent.objects.count()
        pages = (total_events + 49) // 50  # Ceiling division for 50 per page

        self.stdout.write(self.style.SUCCESS(f"📄 Total audit events: {total_events} (will show {pages} pages)"))

        if total_events > 50:
            self.stdout.write(self.style.SUCCESS("🎯 Pagination will now be visible in the audit logs interface!"))
