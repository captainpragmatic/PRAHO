...
def gdpr_dashboard(request: HttpRequest) -> HttpResponse:
    ...
    recent_exports = DataExport.objects.filter(requested_by=user).order_by("-requested_at")[:5]

    # Get recent deletion requests
    recent_deletions = ComplianceLog.objects.filter(compliance_type="gdpr_deletion", user=user).order_by("-timestamp")[
        :5
    ]
...
def request_data_export(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        result = gdpr_export_service.create_data_export_request(
            user=user, request_ip=get_safe_client_ip(request), export_scope=export_scope
        )
        ...
        match result:
            case Ok(export_request):
                ...
                messages.success(
                    request,
                    _(
                        "Your data export request has been created. You will receive an email "
                        "when it is ready for download. Request ID: {}"
                        ...
                        if request.headers.get("HX-Request"):
                            ...
                            if export_request.status == "completed" and export_request.file_path:
                                try:
                                    ...
                                    logger.info(
                                        f"✅ [GDPR Export] Immediate download served for export {export_request.id}"
                                    )
                                    return response

                                except Exception as e:
                                    logger.error(
                                        f"🔥 [GDPR Export] Failed to serve immediate download: {e}", exc_info=True
                                    )
            ...
            case Err(error_msg):
                ...
                messages.error(request, _("Failed to create data export request: {}").format(error_msg))
...
def download_data_export(request: HttpRequest, export_id: uuid.UUID) -> HttpResponse:
    ...
    try:
        ...
        export_request = get_object_or_404(DataExport, id=export_id, requested_by=user, status="completed")
...
def request_data_deletion(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        result = gdpr_deletion_service.create_deletion_request(
            user=user, deletion_type=deletion_type, request_ip=get_safe_client_ip(request), reason=reason
        )
        ...
        match result:
            case Ok(deletion_request):
                messages.warning(
                    request,
                    _(
                        "Your data deletion request has been submitted. This action cannot be undone. Request ID: {}"
                    ).format(deletion_request.reference_id[:16]),
                )
                ...
                if request.POST.get("confirm_immediate") == "yes":
                    ...
                    match processing_result:
                        case Ok(_):
                            messages.success(
                                request, _("Your account data has been processed according to your request.")
                            )
            ...
            case Err(error_msg):
                messages.error(request, _("Failed to create deletion request: {}").format(error_msg))
...
def withdraw_consent(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        result = gdpr_consent_service.withdraw_consent(
            user=user, consent_types=consent_types, request_ip=get_safe_client_ip(request)
        )

        match result:
            case Ok(success_msg):
                messages.success(request, _("Your consent has been withdrawn for: {}").format(success_msg))
                ...
                if "data_processing" in consent_types:
                    messages.warning(
                        request,
                        _(
                            "Data processing consent withdrawal will trigger account anonymization. "
                            "This cannot be undone."
                        ),
                    )
            case Err(error_msg):
                messages.error(request, _("Failed to withdraw consent: {}").format(error_msg))
...
def update_consent(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        logger.info(
            f"🔍 [GDPR Consent] Marketing consent checkbox: {request.POST.get('accepts_marketing')} -> {marketing_consent}"
        )
        ...
        logger.info(
            f"🔍 [GDPR Consent] Data processing consent checkbox: {request.POST.get('gdpr_consent')} -> {data_processing_consent}"
        )
        ...
        if changes_made:
            ...
            messages.success(request, _("Your privacy preferences have been updated successfully."))
        else:
...
def audit_management_dashboard(request: HttpRequest) -> HttpResponse:
    ...
    # Log staff access
    logger.info(
        f"🔒 [Audit Management] Dashboard accessed by {getattr(request.user, 'email', 'anonymous')} from {get_safe_client_ip(request)}"
    )
    ...
    # Active alerts
    active_alerts = AuditAlert.objects.filter(status__in=["active", "acknowledged", "investigating"]).order_by(
        "-created_at"
    )[:10]
    ...
    popular_searches = AuditSearchQuery.objects.filter(is_shared=True).order_by("-usage_count")[:5]
...
def audit_log(request: HttpRequest) -> HttpResponse:
    ...
    context = {
        ...
        "saved_searches": AuditSearchQuery.objects.filter(Q(created_by=request.user) | Q(is_shared=True)).order_by(
            "-last_used_at", "name"
        ),
    }
...
def logs_list(request: HttpRequest) -> HttpResponse:
    ...
    filters = {
        ...
        "content_types": [
            ct_id for ct_id in request.GET.getlist("content_type") if ct_id.strip() and ct_id.strip().isdigit()
        ]
        or None,
...
def _export_logs_json(queryset: models.QuerySet[AuditEvent], timestamp: str) -> HttpResponse:
    ...
    response = HttpResponse(
        json.dumps(data, indent=2, default=str, ensure_ascii=False), content_type="application/json"
    )
...
def gdpr_management_dashboard(request: HttpRequest) -> HttpResponse:
    ...
    logger.info(
        f"🔒 [Staff GDPR] Dashboard accessed by {getattr(request.user, 'email', 'anonymous')} from {get_safe_client_ip(request)}"
    )
    ...
    export_stats = {
        ...
        "expired": DataExport.objects.filter(status="completed", expires_at__lt=timezone.now()).count(),
    }
...
def gdpr_export_detail(request: HttpRequest, export_id: uuid.UUID) -> HttpResponse:
    ...
    context = {
        ...
        "is_expired": (export_request.status == "completed" and timezone.now() > export_request.expires_at),
    }
...
def save_search_query(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        result = audit_search_service.save_search_query(
            name=name, query_params=query_params, user=request.user, description=description, is_shared=is_shared
        )

        match result:
            case Ok(_):
                messages.success(request, _('Search query "{}" saved successfully.').format(name))
            case Err(error_msg):
                messages.error(request, _("Failed to save search query: {}").format(error_msg))
...
def load_saved_search(request: HttpRequest, query_id: uuid.UUID) -> HttpResponse:
    ...
    try:
        search_query = get_object_or_404(AuditSearchQuery, id=query_id)
...
def run_integrity_check(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        result = audit_integrity_service.verify_audit_integrity(
            period_start=start_time, period_end=end_time, check_type=check_type
        )
            ...
            case Ok(integrity_check):
                messages.success(
                    ...
                    _("Integrity check completed: {} ({} issues found)").format(
                        integrity_check.get_status_display(), integrity_check.issues_found
                    ),
                )
            case Err(error_msg):
                messages.error(request, _("Integrity check failed: {}").format(error_msg))
...
def apply_retention_policies(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        if request.POST.get("confirm") != "yes":
            messages.warning(
                request, _("Retention policy application requires confirmation. This action cannot be undone.")
            )
        ...
        match result:
            case Ok(results):
                messages.success(
                    ...
                    _("Retention policies applied: {} policies processed, {} events affected").format(
                        results["policies_applied"], results["events_processed"]
                    ),
                )
            ...
            case Err(error_msg):
                messages.error(request, _("Failed to apply retention policies: {}").format(error_msg))
...
def alerts_dashboard(request: HttpRequest) -> HttpResponse:
    ...
    alert_stats = {
        ...
        "critical_alerts": AuditAlert.objects.filter(
            severity="critical", status__in=["active", "acknowledged", "investigating"]
        ).count(),
...
def event_detail(request: HttpRequest, event_id: uuid.UUID) -> HttpResponse:
    ...
    if event.request_id:
        related_by_request = (
            AuditEvent.objects.filter(request_id=event.request_id).exclude(id=event.id).order_by("timestamp")
        )
    ...
    related_alerts = AuditAlert.objects.filter(related_events=event).order_by("-created_at")[:5]
