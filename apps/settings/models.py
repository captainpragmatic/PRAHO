...
SettingCategoryType = Literal[
    'billing', 'users', 'domains', 'provisioning', 'security', 
    'notifications', 'integrations', 'system'
]
...
class SettingCategory(models.Model):
    ...
    key = models.CharField(
        ...
        max_length=50,
        unique=True,
        help_text=_('Unique category identifier (e.g., "billing")')
    )
    ...
    description = models.TextField(
        _('Description'),
        blank=True,
        help_text=_('Category description for staff')
    )
    ...
    created_at = models.DateTimeField(
        ...
        help_text=_('When this category was created')
    )
    
    updated_at = models.DateTimeField(
        _('Updated At'),
        auto_now=True,
        help_text=_('When this category was last updated')
    )
...
class SystemSetting(models.Model):
    ...
    value = models.JSONField(
        _('Value'),
        help_text=_('Current setting value')
    )
    
    default_value = models.JSONField(
        _('Default Value'),
        help_text=_('Default value to use if setting is not configured')
    )
    ...
    is_public = models.BooleanField(
        _('Is Public'),
        default=False,
        help_text=_('Whether this setting can be accessed publicly')
    )
    
    requires_restart = models.BooleanField(
        _('Requires Restart'),
        default=False,
        help_text=_('Whether changing this setting requires application restart')
    )
    ...
    help_text = models.TextField(
        _('Help Text'),
        blank=True,
        help_text=_('Additional help text for staff users')
    )
    
    created_at = models.DateTimeField(
        _('Created At'),
        default=timezone.now,
        help_text=_('When this setting was created')
    )
    
    updated_at = models.DateTimeField(
        _('Updated At'),
        auto_now=True,
        help_text=_('When this setting was last updated')
    )
    ...
    def clean(self) -> None:
        ...
        if self.key and '.' not in self.key:
            raise ValidationError({
                'key': _('Setting key must be in format "category.setting_name"')
            })
        ...
        if self.is_required and self.value is None:
            raise ValidationError({
                'value': _('Required settings must have a value')
            })
        ...
        try:
            if self.data_type == 'string':
                if not isinstance(value, str):
                    raise ValidationError({
                        field_name: _('Value must be a string')
                    })
                ...
                if not isinstance(value, bool):
                    raise ValidationError({
                        field_name: _('Value must be a boolean')
                    })
            elif self.data_type == 'decimal':
                if not isinstance(value, int | float | str | Decimal):
                    raise ValidationError({
                        field_name: _('Value must be a decimal number')
                    })
                Decimal(str(value))  # Validate decimal conversion
            elif self.data_type == 'list':
                if not isinstance(value, list):
                    raise ValidationError({
                        field_name: _('Value must be a list')
                    })
    ...
    def get_typed_value(self) -> str | int | bool | Decimal | list | dict | None:
