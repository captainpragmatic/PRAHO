...
def handle_setting_saved(sender: Any, instance: SystemSetting, created: bool, **kwargs: Any) -> None:
    ...
    try:
        ...
        logger.error(
            "🔥 [Settings Signal] Error handling save for setting %s: %s",
            instance.key,
            str(e)
        )
...
def handle_setting_deleted(sender: Any, instance: SystemSetting, **kwargs: Any) -> None:
    ...
    try:
        ...
        logger.warning(
            "⚠️ [Settings Signal] Setting %s deleted",
            instance.key
        )
        ...
        logger.error(
            "🔥 [Settings Signal] Error handling deletion for setting %s: %s",
            instance.key,
            str(e)
        )
...
def _send_critical_setting_notification(setting: SystemSetting, action: str) -> None:
    ...
    try:
        ...
        if NotificationService is None:
            logger.debug(
                "⚠️ [Settings Signal] Notifications service not available for %s",
                setting.key
            )
            return
        ...
        logger.info(
            "📧 [Settings Signal] Critical setting notification sent for %s %s",
            setting.key,
            action
        )
        ...
        logger.error(
            "🔥 [Settings Signal] Error sending notification for %s: %s",
            setting.key,
            str(e)
        )
