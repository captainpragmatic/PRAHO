...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS('✅ [Setup] Created provisioning category')
            )
        else:
            self.stdout.write(
                self.style.WARNING('⚠️ [Setup] Provisioning category already exists')
            )
        ...
        for setting_data in virtualmin_settings:
            ...
            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'✅ [Setup] Created {setting_data["key"]}')
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f'⚠️ [Setup] {setting_data["key"]} already exists')
                )
