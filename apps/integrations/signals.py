...
def log_webhook_reliability_events(sender: type[WebhookEvent], instance: WebhookEvent, created: bool, **kwargs: Any) -> None:
    """
    ...
    try:
        ...
        # Log skipped webhooks (duplicates detected)
            ...
            logger.info(f"📋 [Integrations] Webhook skipped: {instance.source}.{instance.event_type} - {instance.error_message}")
...
def log_outbound_webhook_events(sender: type[WebhookDelivery], instance: WebhookDelivery, created: bool, **kwargs: Any) -> None:
    """
...
def _log_webhook_success(webhook_event: WebhookEvent, response_time_ms: int) -> None:
    ...
    try:
        ...
        logger.info(f"✅ [Integrations] Webhook processed: {webhook_event.source}.{webhook_event.event_type} ({response_time_ms}ms, {performance_grade})")
...
def _log_webhook_failure(webhook_event: WebhookEvent, is_retry: bool = False) -> None:
    ...
    try:
        ...
        reliability_context = {
            ...
            'sla_breach': webhook_event.retry_count >= WEBHOOK_RELIABILITY_THRESHOLDS['sla_breach_retry_threshold'] and service_priority == 'critical',
        }
        ...
        log_level(f"❌ [Integrations] Webhook failed: {webhook_event.source}.{webhook_event.event_type} - {error_type}{retry_info}")
...
def _log_webhook_retry_exhausted(webhook_event: WebhookEvent) -> None:
    ...
    try:
        ...
        if service_priority == 'critical':
            logger.critical(f"🚨 [Integrations] CRITICAL webhook failure: {webhook_event.source}.{webhook_event.event_type} - REQUIRES IMMEDIATE ATTENTION")
        else:
            logger.error(f"🔥 [Integrations] Webhook retry exhausted: {webhook_event.source}.{webhook_event.event_type} after {webhook_event.retry_count} attempts")
...
def _log_outbound_webhook_result(webhook_delivery: WebhookDelivery) -> None:
    ...
    try:
        if webhook_delivery.status == 'delivered':
            ...
            logger.info(f"📤 [Integrations] Outbound webhook delivered: {webhook_delivery.customer} - {webhook_delivery.event_type} ({response_time_ms}ms)")
            
        elif webhook_delivery.status == 'failed':
            logger.warning(f"📤 [Integrations] Outbound webhook failed: {webhook_delivery.customer} - {webhook_delivery.event_type} (retry {webhook_delivery.retry_count})")
