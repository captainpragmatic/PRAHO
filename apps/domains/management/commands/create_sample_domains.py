...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: Any) -> None:
        parser.add_argument(
            ...
            type=int,
            default=5,
            help='Number of domains to create (default: 5)'
        )
    ...
    def create_sample_tlds(self) -> int:
        ...
        for tld_data in tlds_data:
            tld, created = TLD.objects.get_or_create(
                extension=tld_data['extension'],
                defaults=tld_data
            )
            if created:
                created_count += 1
    ...
    def create_sample_registrars(self) -> int:
        ...
        for registrar_data in registrars_data:
            registrar, created = Registrar.objects.get_or_create(
                name=registrar_data['name'],
                defaults=registrar_data
            )
            if created:
                created_count += 1
    ...
    def create_sample_domains(self, count: int, customers: list[Customer]) -> int:
        ...
        for i in range(count):
            ...
            try:
                assignment = TLDRegistrarAssignment.objects.get(
                    tld=tld, 
                    is_primary=True
                )
