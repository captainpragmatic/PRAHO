...
class RegistrarWebhookView(View):
    ...
    def _process_webhook_event(
        ...
        if event_type == 'domain.registered':
            return self._handle_domain_registered(domain, webhook_data, client_ip)
        elif event_type == 'domain.renewed':
            return self._handle_domain_renewed(domain, webhook_data, client_ip)
        elif event_type == 'domain.transfer.completed':
            return self._handle_domain_transfer_completed(domain, webhook_data, client_ip)
        elif event_type == 'domain.expiring':
            return self._handle_domain_expiring(domain, webhook_data, client_ip)
        elif event_type == 'domain.expired':
            return self._handle_domain_expired(domain, webhook_data, client_ip)
        elif event_type == 'domain.suspended':
            return self._handle_domain_suspended(domain, webhook_data, client_ip)
        elif event_type == 'whois.privacy.enabled':
            return self._handle_whois_privacy_changed(domain, webhook_data, True, client_ip)
        elif event_type == 'whois.privacy.disabled':
            return self._handle_whois_privacy_changed(domain, webhook_data, False, client_ip)
        else:
            logger.warning(f"⚠️ [Webhook] Unknown event type: {event_type}")
            return True, f"Event {event_type} acknowledged (no handler)"
    
    def _handle_domain_registered(self, domain: Domain, webhook_data: dict[str, Any], client_ip: str) -> tuple[bool, str]:
    ...
    def _handle_domain_suspended(self, domain: Domain, webhook_data: dict[str, Any], client_ip: str) -> tuple[bool, str]:
