...
def cleanup_expired_2fa_sessions() -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            inactive_webauthn = WebAuthnCredential.objects.filter(
                is_active=False,
                created_at__lt=old_webauthn_cutoff
            )
...
def audit_suspicious_login_patterns() -> dict[str, Any]:
    ...
    try:
        ...
        suspicious_users_query = UserLoginLog.objects.filter(
            timestamp__gte=cutoff,
            status__in=['failed_password', 'failed_2fa']
        ...
        suspicious_ips_query = UserLoginLog.objects.filter(
            timestamp__gte=cutoff,
            status__in=['failed_password', 'failed_user_not_found']
        ...
        for login in recent_successful_logins:
            if login.user:
                ...
                previous_countries = UserLoginLog.objects.filter(
                    user=login.user,
                    status='success',
