...
def prepare_billing_table_data(documents: list[Any], user: Any) -> dict[str, list[dict[str, Any]]]:
    ...
    for doc in documents:
        ...
        document_cell = {
            ...
            "url": reverse("billing:invoice_detail", args=[doc_id])
            if doc_type == "invoice"
            else reverse("billing:proforma_detail", args=[doc_id]),
        }
        ...
        # Amount cell
        amount_cell = {"content": f"<div class='text-white font-medium'>{total_amount:.2f} RON</div>"}
...
def prepare_orders_table_data(orders_page: Any, user: Any) -> dict[str, Any]:
    ...
    for order in orders_page.object_list:
        ...
        rows.append({"clickable": True, "click_url": detail_url, "cells": cells, "actions": actions})
