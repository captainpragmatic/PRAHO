...
MONITORING_WINDOW_MINUTES = 15  # Time window for pattern detection
SUSPICIOUS_UPLOAD_COUNT = 10  # File uploads per window that trigger alert
...
class SecurityEventTracker:
    ...
    def __init__(self) -> None:
        ...
        self.alert_threshold = SECURITY_ALERT_THRESHOLD
    ...
    def track_failed_access(self, user_id: int, resource_type: str, resource_id: str, reason: str) -> None:
        ...
        if len(recent_failures) >= self.alert_threshold:
            self._trigger_security_alert(user_id, 'repeated_access_failures', {
    ...
    def track_file_upload(self, user_id: int, filename: str, file_size: int, scan_result: str) -> None:
        ...
        cutoff_time = timezone.now() - timedelta(minutes=self.monitoring_window)
        recent_uploads = [
            u for u in current_uploads 
            if timezone.datetime.fromisoformat(u['timestamp']) > cutoff_time
        ]
    ...
    def track_privilege_escalation_attempt(self, user_id: int, attempted_action: str, target_resource: str) -> None:
        ...
        self._trigger_security_alert(user_id, 'privilege_escalation_attempt', {
    ...
    def _analyze_upload_patterns(self, user_id: int, uploads: list[dict[str, Any]]) -> None:
        ...
        if len(failed_scans) >= FAILED_SCAN_THRESHOLD:
            self._trigger_security_alert(user_id, 'repeated_scan_failures', {
                'failed_scan_count': len(failed_scans),
                'recent_failures': failed_scans[-FAILED_SCAN_THRESHOLD:]
            })
    ...
    def _trigger_security_alert(self, user_id: int, alert_type: str, alert_data: dict[str, Any]) -> None:
        ...
        cache.set(alert_key, {
            ...
            'timestamp': timezone.now().isoformat()
        }, 60 * 60 * 24)  # Keep alerts for 24 hours
    ...
    def _get_top_security_events(self, cutoff_time: datetime) -> list[dict[str, Any]]:
        ...
        return [
            {
                'event_type': 'file_upload_failures',
                'count': self._get_scan_failure_count(cutoff_time)
            },
            {
    ...
    summary = {
        ...
        'time_ranges': {
            ...
            'last_7d': {
                ...
                'unique_uploaders': TicketAttachment.objects.filter(
                    uploaded_at__gte=last_7d
        ...
        'top_file_types': list(TicketAttachment.objects.filter(
            uploaded_at__gte=last_7d
        ).values('content_type').annotate(count=Count('content_type')).order_by('-count')[:5]),
