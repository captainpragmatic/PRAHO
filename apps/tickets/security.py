...
class FileSecurityScanner:
    ...
    def scan_uploaded_file(self, uploaded_file: UploadedFile, original_filename: str | None = None) -> tuple[bool, str]:  # noqa: PLR0911
        ...
        try:
            ...
            if not self._validate_file_size(uploaded_file):
                return self._reject_file(f"File too large ({uploaded_file.size} bytes > {MAX_FILE_SIZE_BYTES})", filename)
    ...
    def _validate_file_size(self, uploaded_file: UploadedFile) -> bool:
        ...
        if uploaded_file.size > MAX_FILE_SIZE_BYTES:
            logger.warning(f"🚨 [Ticket Security] File too large: {uploaded_file.size} bytes")
            return False
        ...
        try:
            ...
            for pattern in SUSPICIOUS_PATTERNS:
                if pattern in content_lower:
                    logger.warning(f"🚨 [Ticket Security] Suspicious pattern detected: {pattern.decode('utf-8', errors='ignore')}")
                    return False
