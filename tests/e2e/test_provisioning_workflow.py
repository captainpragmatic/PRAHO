...
class TestServiceProvisioningWorkflow(TestCase):
    ...
    def test_order_to_provisioning_flow(self):
        ...
        OrderItem.objects.create(
            ...
            product_sku=self.hosting_product.sku,
            quantity=1,
    ...
    def test_hosting_product_provisioning_requirements(self):
        ...
        assert self.hosting_product.sku is not None
        assert self.hosting_product.price_cents > 0
        assert self.hosting_product.billing_cycle == 'monthly'
    ...
    def test_order_with_multiple_services(self):
        ...
        OrderItem.objects.create(
            ...
            product_sku=domain_product.sku,
            quantity=1,
...
class TestProvisioningWithDomain(TestCase):
    ...
    def setUp(self):
        ...
        self.category, _ = ProductCategory.objects.get_or_create(
            name='Domains',
            defaults={'slug': 'domains', 'is_active': True}
        )
    ...
    def test_domain_registration_order(self):
        ...
        OrderItem.objects.create(
            ...
            unit_price_cents=4500,
            total_cents=4500,
            meta={'domain_name': 'test-domain.ro'},
        )

        assert order.items.count() == 1
        item = order.items.first()
        assert item.meta.get('domain_name') == 'test-domain.ro'
...
class TestBundleProvisioning(TestCase):
    ...
    def setUp(self):
        ...
        self.category, _ = ProductCategory.objects.get_or_create(
            name='Bundles',
            defaults={'slug': 'bundles', 'is_active': True}
        )
    ...
    def test_hosting_bundle_order(self):
        ...
        bundle_product = Product.objects.create(
            ...
            price_cents=14900,  # Discounted bundle price
            currency_code='RON',
            billing_cycle='yearly',
            is_active=True,
            category=self.category,
        ...
        assert order.items.count() == 1
        item = order.items.first()
        assert len(item.meta.get('bundle_components', [])) == 3
